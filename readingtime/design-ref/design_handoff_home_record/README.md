...
12. 마지막 기록 행 탭 → **종이책 케이스에서** 그 책 상세(08). 소연 행 탭 → 파트너 통계.
12b. **밀리가 최신인 케이스는 이번 작업 범위 밖이다.** 행에는 밀리 책 제목이 뜨는데 탭하면 `openRecentDetail()` 이 마지막 *종이* 세션의 책을 열어 서로 다른 책이 열린다. `09ffefd` 이전부터 있던 기존 결함이고 §4-⑥ 이 이 행을 "기존 유지"로 두었으므로 **고치지 않는다.** AC 로 검증하지도 말 것 — 별건으로 기록만 남긴다(모델 라우팅을 건드려야 하며 홈 리디자인과 무관하다).
13. 연속 1일째에 `최고까지 0일`이 뜨지 않는다(진행 구간 제외 계산 검증).
14. 신기록 상태에서 게이지가 골드로 바뀌고 `+N일`이 표시된다.

접근성

15. **색면이 깔린 칸의 숫자만 4.5:1 이상이다** — 읽은 과거 `#2E1C15`(α 0.256 에서 11.2:1, α 0.72 에서 5.6:1), 오늘 흰 글자 4.51:1. 이 셋은 이번 작업이 새로 만든 조합이라 반드시 통과해야 한다.
15b. **미기록·미래 칸의 숫자는 4.5:1 대상이 아니다.** `Screen11Month.numColor` 의 앱 전역 관례를 그대로 쓰며 실측은 미기록 평일 `#B5AD97` 2.18:1 · 미래 `#D3CBB6` 1.58:1 · 미기록 일요일 terra 4.40:1 이다. 이 값을 올리려 색을 바꾸지 말 것 — 홈과 월간(11)이 어긋나고 월간의 픽셀 오라클이 깨진다.
15c. 그 대신 **색이 유일한 정보 채널이 아님**을 확인한다(WCAG 1.4.1): 읽었는지는 **칩의 유무**로, 얼마나는 **칩의 농도**로 전달되며, 칸마다 VoiceOver 라벨이 날짜와 분을 읽는다(§8-3). 날짜 숫자는 정보가 아니라 저강조 길잡이다.

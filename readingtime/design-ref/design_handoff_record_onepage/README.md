...
- **2026-09-02 갈라지지 않는 클러스터**: 핀 탭 규칙(§5 클러스터=줌투핏)의 예외 — 구성원이 250m 안이거나 이미 줌투핏
  하한(0.03°)이면 구성원을 합친 place 시트(제목 `앵커 외 N`)를 연다. 또 120m 안의 다른 장소 키(역지오코딩 실패 좌표 키)는
  같은 장소로 병합한다. 실기기에서 "서교동 외 1"(30m 차이)을 눌러도 시트가 영영 안 열리던 결함.

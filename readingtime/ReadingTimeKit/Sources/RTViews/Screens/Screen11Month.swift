...
public struct Screen11Month: View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(alignment: .bottom) {
                    Text("2026년 5월").font(.sans(26, 900)).tracking(26 * -0.04).foregroundColor(RT.ink)
                    Spacer()
                    HStack(spacing: 8) {
                        navBtn(back: true)
                        navBtn(back: false)
    ...
    var summary: some View {
        HStack(spacing: 10) {
            HStack(spacing: 10) {
                Text("21:08").font(.mono(17, 700)).tracking(17 * -0.02).foregroundColor(RT.ink)
                Text("총 시간").font(.sans(12, 500)).foregroundColor(RT.muted)
            }
            Circle().fill(Color(hex: 0xD5CDB8)).frame(width: 3, height: 3)
            HStack(spacing: 0) {
                Text("17").font(.mono(12, 700)).foregroundColor(RT.green)
                Text(" / 21일 읽음").font(.sans(12, 500)).foregroundColor(RT.muted)
            }
            Circle().fill(Color(hex: 0xD5CDB8)).frame(width: 3, height: 3)

...
// 9/17 에 요일 원이 날짜만 보이는 2주 카드로 바뀐 뒤, 트레드밀 화면에는 지난 기록 수치가
// 하나도 남지 않았다. 근력은 칸 = 세트, 유산소는 세션당 한 번이라 칸 = 세션이다.
@Suite struct CardioRecordBarTests {
    ...
    @Test func pastIsLastFourRunsOldestFirst() {
        ...
        #expect(bar.past.map(\.bottom) == ["15분", "16분", "17분", "18분"])
    }

    // 거리가 빠진 날은 0 이 아니라 "—" — 기록이 비었다는 사실을 그대로 보여준다.
    @Test func missingDistanceIsDashNotZero() {
        let bar = GymSessionLogic.cardioRecordBar(history: history, exerciseId: "treadmill", todaySet: nil)
        #expect(bar.past[2] == .init(top: "—", bottom: "17분", distanceKm: 0))
    }
    ...
    @Test func todayShowsEnteredValuesOrDash() {
        ...
        #expect(empty.today == .init(top: "—", bottom: "", distanceKm: 0))
        ...
        #expect(typed.today == .init(top: "2.2", bottom: "19분", distanceKm: 2.2))
    }
    ...
    @Test func bestIsLongestDistanceIncludingToday() {
        ...
        #expect(bar.best == .init(top: "3.0", bottom: "30분", distanceKm: 3.0))
        ...
        #expect(beat.best == .init(top: "3.2", bottom: "31분", distanceKm: 3.2))
    }

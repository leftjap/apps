...
public enum GymScreens {
    ...
    // 거리를 안 적은 유산소 날이 섞인 홈 — 원 안이 "—" 로 뜨는지 본다 (2026-09-17).
    // 실기기 실측에서 최근 6주 중 3주에 거리 0 인 날이 한 건씩 있었다(24건 중 3건).
    // 오늘 = 2026-09-17(목). 이번 주 월 1.6km, 지난주 화 1.5 · 목 1.6 · 금 거리미기록.
    @MainActor static func demoCardioDashModel() -> GymAppModel {
        func run(_ date: String, _ min: Double, _ km: Double?) -> GymSession {
            GymSession(id: "cd-\(date)", date: date, startTime: 1_758_000_000_000,
                       blocks: [GymBlock(exerciseId: "treadmill", sets: [
                           GymSet(done: true, duration: min * 60, distance: km)])],
                       tags: ["cardio"], status: .completed)
        }
        let m = GymAppModel(snapshotSession: GymSession(id: "dash", date: "2026-09-17", status: .active))
        if let d = GymAppModel.dayFmt.date(from: "2026-09-17") { m.referenceToday = d }
        m.history = [run("2026-09-14", 15, 1.6),
                     run("2026-09-08", 15, 1.5), run("2026-09-10", 16, 1.6),
                     run("2026-09-11", 14, nil)]     // 거리만 안 적은 날 → "—"
        healthySync(m); return m
    }

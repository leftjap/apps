#!/usr/bin/env bash
# Wave 11.7 — supabase/migrations/0001_gym_init.sql 을 클립보드에 복사.
# 사용: pnpm sql:copy
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
SQL="$HERE/../supabase/migrations/0001_gym_init.sql"
...
echo "     where schemaname='public' and tablename like 'gym_%';"
echo "     → 4 row, 모두 rowsecurity=true 이어야 정상"

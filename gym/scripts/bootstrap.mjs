#!/usr/bin/env node
/**
 * Wave 11.7 — 종합 셋업 진단 + 가이드 (Study 11.12 패턴).
 *
 * 사용: pnpm bootstrap
 *
 * 로직:
...
if (!existsSync(ENV)) {
  ...
  console.log('   (Study 셋업이 끝났다면 ~/apps/study/.env.local 의 두 값 그대로 복사)');
  process.exit(1);
}
...
if (!url || !key) {
  ...
  console.log('   2. Project URL / publishable key 복사하여 .env.local 에 붙여넣기');
...
console.log('   → DevTools → Application → IndexedDB → gym_<12hex> 확인');
console.log('');
console.log('Google OAuth Provider 가 미설정이면 pnpm oauth:guide 참고.');

...
final class GymCardioUITests: XCTestCase {
    ...
    func testCardioGhostHeroIsLabeledNotRecorded() {
        ...
        XCTAssertTrue(cardioLabel(app, startsWith: "거리").contains("미입력"),
                      "직전 기록도 입력도 없으면 미입력 (실측 '\(cardioLabel(app, startsWith: "거리"))')")

        // ② 거리 입력 → 라벨에서 미입력 표시가 사라진다. 거리는 소수 1자리 고정이라 "15.0".
        enterHero(app, keys: ["1", "5"])
        XCTAssertEqual(heroValue(app).label, "15.0", "히어로에 입력값이 떠야")
        XCTAssertEqual(cardioLabel(app, startsWith: "거리"), "거리",
                       "입력값은 고스트가 아니다 (실측 '\(cardioLabel(app, startsWith: "거리"))')")

...
- [/] (f-5-2) reorder 모드 시각 토글 — 본 세션 (`setReorderMode(doc, on, srcIdx)` export — src pill scale(1.05) + accent halo shadow + zIndex 10 / 다른 pill scale(0.92) + opacity 0.55 + transition 200ms + reorder dataset 마커 + 빈 영역 click capture cancel + 액션 시트/키패드 영역 click 무시 가드. handleActionSelect 의 active-card.reorder 와 footer-exercise.reorder 가 setReorderMode(true, blockIdx) 호출. wireLongPress 의 scale 복원을 onTrigger **전** 으로 이동 (이전 200ms 후 cleanup 이 setReorderMode 의 transform 을 덮어쓰는 race 해결). drag/drop 으로 실제 위치 변경 = (f-5-3) 후속)

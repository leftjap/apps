...
- [x] (f-5-1) footer pill 실 데이터 wiring — 본 세션 (mocks footer 정적 4 pill 제거 → `sessionFooterPills` 동적 영역만. session.js 에 `_currentBlockIdx` 모듈 변수 + `classifyBlockState` (current/done/hold/pending) + `blockProgressText` (·N/M / ·N세트) + `blockDisplayName` + `renderFooterPillHtml` (spec §6-8 footer nav 표현) + `renderFooterPills` (active session blocks 기반) + `wireFooterPillClick` (data-block-idx → _currentBlockIdx 갱신 + mountSessionView 재바인딩) + `getCurrentBlockAndCursor`/mountSessionView 의 active branch 가 _currentBlockIdx 우선 사용 + `handleActionSelect` 의 footer-exercise 진짜 핸들러 (active finish/completed edit/delete/reorder). circuit 블록도 round1 entry done 비율로 상태 판정)
- [x] (f-5-2) reorder 모드 시각 토글 — 본 세션 (`setReorderMode(doc, on, srcIdx)` export — src pill scale(1.05) + accent halo shadow + zIndex 10 / 다른 pill scale(0.92) + opacity 0.55 + transition 200ms + reorder dataset 마커 + 빈 영역 click capture cancel + 액션 시트/키패드 영역 click 무시 가드. handleActionSelect 의 active-card.reorder 와 footer-exercise.reorder 가 setReorderMode(true, blockIdx) 호출. wireLongPress 의 scale 복원을 onTrigger **전** 으로 이동 (race 해결))
- [x] (f-5-3) drag/drop 으로 실제 위치 변경 — 본 세션:
  - (f-5-3a) wireReorderDrag (pointerdown/move/up/cancel + idempotent spaDragHooked + setPointerCapture). pointermove 마다 dragPill.style.transform = scale(1.05) translate(dx,dy). bubble fix — wireLongPress 의 stopPropagation 제거 + target 검사 대체 (pillsEl bubble 회복).
  - (f-5-3b) `computeDropIdx(pillsEl, clientX, srcIdx)` (export) — src 제외 다른 pill center.x 비교 + 가장 가까운 idx (clientX < center → idx 앞 자리, > center → 그 뒤). `applyDropPlaceholder` (dst idx pill marginLeft 18px / dst idx+1 pill marginRight 18px). `clearDropPlaceholder` (pointerup/cancel cleanup).
  - (f-5-3c) `performBlockReorder(srcIdx, dstIdx)` (export) — blocks.splice(srcIdx,1) + splice 후 dst 보정 (dst > src 면 insertIdx = dst-1) + blocks.splice(insertIdx,0,removed) + upsertSession + _currentBlockIdx 보정 (src 이동 시 insertIdx, src~dst 사이 cur 는 ±1). wireReorderDrag.pointerup 에서 srcIdx ≠ dstIdx 면 호출 후 mountSessionView. mountSessionView active branch 가 default 선택 시 `_currentBlockIdx = pickedIdx` 동기화 추가 (reorder 후 cardExName + footer active 모두 일관).

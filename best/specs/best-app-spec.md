...
- **기간 필터 축은 기간마다 다르다** (`logic.js periodFilter`, 2026-07-11 실측 정정):
  - **일간 = `collected_on = 최신 수집일`** (최신 스냅샷 전체). 수집은 하루 1회(08:00)인데 인기글은 대부분 *전날* 게시된다 → `posted_at >= 오늘0시` 로 자르면 **새벽엔 0건**이 된다(실측 버그). 스냅샷 자체를 일간으로 본다.
  - **주/월/연 = `posted_at >= (최신일 - N일)`**. 소급 복구분도 실제 게시일로 배치되고, 7일 이상 창이라 새벽에도 안 빈다. `collected_on` 으로 하면 14일 소급분이 '오늘'로 몰려 왜곡되므로 여기선 `posted_at` 이 맞다.
  - 보배 직접 크롤도 목록 날짜 셀을 파싱해 `posted_at` 을 채운다 (당일 `HH:MM`, 이전일 `MM/DD` — 2026-07-10 p1·p15 실측).
  - 검증: 2026-07-11 02:43 실측 일 2,014 / 주 2,933 / 월·연 3,905 (DB count 와 화면 일치, 단조 증가, 빈 상태 없음).

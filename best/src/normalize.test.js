...
describe('normalize — best_posts 행 변환', () => {
  ...
  it('이슈링크 비보배 사이트: board null, post_key 는 goId', () => {
    const post = fromIssuelink(
      { goId: '468400010030932', title: '[쿠팡] 왕뚜껑 국물라볶이 4개', comments: 0, views: 10, postedAt: null, url: 'u' },
      { site: 'ppomppu', collectedOn: '2026-07-10' },
    )
    expect(post).toMatchObject({ site: 'ppomppu', board: null, post_key: '468400010030932', posted_at: null, is_ad: true })
  })
  ...
  it('dcbest: 갤러리 태그가 board, 보배 직접: best 판 자체 채번이므로 post_key 는 best:No', () => {
    ...
    const bb = fromBobae(
      { no: '1008318', title: 't', boardName: '유머게시판', boardCode: 'strange', comments: 34, views: 3170, url: 'u' },
      { collectedOn: '2026-07-10' },
    )
    expect(bb).toMatchObject({ source: 'bobae', site: 'bobae', board: 'strange', post_key: 'best:1008318', posted_at: null })
  })
})

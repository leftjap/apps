...
export function createIndexedDBStorage({ legacyLocalStorageKey } = {}) {
  ...
  return {
    ...
    async removeItem(key) {
      try { await idbOp('readwrite', (s) => s.delete(key)); } catch {}
      if (typeof localStorage !== 'undefined') { try { localStorage.removeItem(key); } catch {} }
    },
  };
}

export const __resetDBPromiseForTests = async () => {
  if (_dbPromise) {

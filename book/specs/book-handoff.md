...
- **남은 placeholder 2권**:
  ...
  - **book_095** ⚠ **오염 entry(6책 혼재)** — 어구록 21건이 6개 책에서 옴. Naver(실Chrome AI브리핑)+Aladin+Google+WebFetch 전수 검색 결과:
    ...
    - ✅ **홍세화 4건 → book_104 분리**("결: 거칢에 대하여"/홍세화/한겨레 2020, 표지). Naver 2회 일관 + 장발장은행장=홍세화 확정. 저자 확실, 책 유력.
    - ❌ 밀/건강 1·회의주의(미신) 2·사유/인지 4·계획/실행 4 + 미상 1 = 12건 → **generic 산문이라 Google·Naver(실Chrome AI브리핑)·Aladin·verbatim 전부 0건**. 어구록 텍스트가 웹 미색인 → **검색으로 식별 불가**(실물 책 필요).
    - **잔여 book_095 = 12건**(미식별) → 라벨을 "(출처 미확인 어구록)/미상"으로 정직 변경(기존 "편하게 사는 삶…/홍세화"는 실존 책 아님). 실물 책 확인 시 추가 분할 가능.

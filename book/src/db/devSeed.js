 ...
 *
 * v14 design-ref/data.jsx 의 QUOTES(15)+comments 를 Dexie 에 주입.
 *  - who:'me' → meId owner, who:'y' → partnerId owner (spec §3.4).
...
/**
 * v14 QUOTES → Dexie 시드 (멱등).
 * @param {{meId: string, partnerId?: string}} opts who:'me'→meId, who:'y'→partnerId
 */
export async function seedDemoData({ meId, partnerId } = {}) {
  ...
  const ownerOf = (who) => (who === 'y' ? (partnerId || meId) : meId);

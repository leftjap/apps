...
export function validateSeedContent(payload, { existingSeeds = [], speakerNames = new Set(), sourceEnLines = null, core100Keys = [] } = {}) {
  ...
  if (!isScenelessTrack && scenes.length !== 1) errors.push(`scene 카드는 정확히 1장이어야 함 (현재 ${scenes.length})`);
  ...
  if (!isScenelessTrack && exprs.length > 3) warnings.push(`표현 ${exprs.length}장 — PPP 집중 추출은 1~2개 권장 (과다 추출 — 핵심만 깊이)`);
  ...
  } else if (isRealClass && !isScenelessTrack) {
  ...
  for (const c of exprs) {
    ...
    if (!isScenelessTrack && chunkW.length >= 2 && PARTICLES.has(chunkW[chunkW.length - 1]) && !hasBasicVerb) {
  ...
  /* ── 음차 연음 게이트는 폐기했다 (2026-09-08 사용자 결정) ──
   * 종전엔 '받침 있는 음절 + 공백 + ㅇ 초성' 을 정규식으로 찾아 "연음 미적용" 경고를 냈다.
   * 폐기 사유: (1) guide-en §7 의 현재 상위 규정이 "현상을 미리 정해 전역 적용하지 않는다,
   * 카드의 실제 발화로 판단한다" 인데 이 게이트가 그 반대를 코드로 강제했다. (2) 한글 철자
   * 패턴으로 음성 현상을 판정하는 구조 자체가 틀렸다 — 앱 음원에서 실제로 끊어지는 자리
   * (2026-09-08 실측: 코어100 의 wait+and, that+isnt 등)까지 '이어 적으라' 고 경고했다.
   * (3) 이 경고가 저작 방향을 끌어당겨, 측정하기 쉬운 t/d 자리만 손보고 어휘 축약·약음은
   * 통과시키는 편향을 만들었다.
   * 판정은 문서 규칙과 카드 음원 대조에 맡긴다. 게이트는 두지 않는다. */

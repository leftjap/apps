...
대표적이면 기본형에 들어간다. `but I → 버라이`, `let me → 레미`, `could you → 쿠쥬`, 비강세 `you → 여` 가 그 예다.
...
— 정상 회화의 대표 realization 이면 기본형. 현행이 이미 `쿠쥬`·`디쥬`·`쏘츄` 로 반영한 것을 되돌리지 않는다.
...
**연쇄 축약 → FAST** — `what do you → 워러유`, `what are you → 워러유`, 문두 `Do you → 쥬`, `how do you → 하우쥬`
처럼 여러 축약이 겹쳐 청감이 크게 바뀌는 형태만 FAST. 다만 사용자는 `두 유` 도 weak you 를 반영한
`두여`·`쥬` 계열이 **기본형 후보에 들어가야** 한다고 했다. 즉 `두 유 → 쥬` 자체는 FAST 가 아닐 수 있다.
문장별로 정한다.

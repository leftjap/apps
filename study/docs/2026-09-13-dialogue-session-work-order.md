...
- **문장 공급원 우선순위 (2026-09-19 사용자 확정, 2026-09-20 자리별 순위 확정)**: 카드 문장의 공급원은 **영상 30패턴(`FzW2gVf7SCw`) · 모두영어 529문장 · 233 뱅크** 셋뿐이고, 지금 단계에서는 이 밖으로 나가지 않는다. 지오 줄 한 자리에 여러 후보가 자연스럽게 들어갈 때의 순위는 **① 30패턴이 든 모두영어 문장 그대로 → ② 모두영어 문장 그대로(30패턴 아님) → ③ 30패턴 자작 → ④ 233 뱅크 자작**. 모두영어 문장이 짧고 빈도가 높아 30패턴 자작보다 앞선다(사용자 결정 2026-09-20). 30패턴은 할당량이 아니라 우선순위이므로 억지로 넣지 않는다. "맥락이 부자연스러워서" 는 진짜일 때만 쓰는 예외이지 빠져나가는 단서가 아니다. 결과 기록: 8판은 모두영어 0/20 이라 폐기, 9판 1~5편은 모두영어 그대로 12/20(`~/apps/tmp/2026-09-19-personal-dialogues-v9.md`), 6~10편은 14/20(`-v9b.md`). 2묶음(10판, `~/apps/tmp/2026-09-20-personal-dialogues-v10.json`)은 40장 중 23장이 모두영어 그대로이고 등급은 a1·b22·c16·d1 이다(저작 직후 21장이었고, 자가 검토에서 등급 c 두 자리에 모두영어 후보가 있었음을 찾아 교체했다). 등급·30패턴·모두영어 일치는 `scripts/dialogue/check-sources.mjs <json>` 으로 센다.
- **소재·순위 재결정 (2026-09-26, 사용자)**: 장면은 일기가 아니라 교재형 일반 상황(공항·기내·호텔·교통·주문·쇼핑·텍스 리펀·스몰토크·외국인 길 안내·전화 예약·병원 접수)을 **인물 시트**에 맞게 창작한다. "장면 = 일기 한 편" 과 "뱅크에 없는 사실 금지" 는 폐기하고, 실제 인물·동물에 대한 사실만 시트와 일치시킨다(나니 시력·호야 사망·소연 직업). 지오 줄은 상황을 말하는 문장이어야 하고 반응형은 편당 1~2줄. 공급원 순위는 **① 영상 30패턴 → ② 모두영어 그대로 → ③ 233 뱅크** 로 되돌리고 2026-09-20 의 "모두영어 그대로 > 30패턴 자작" 은 폐기한다 — 그 순위가 2묶음을 반응 문장 모음으로 만들었다(모두영어 23/40 인데 상황을 말하는 지오 줄이 거의 없음). 재사용은 뜻이 같은 후보가 있을 때의 선택 기준이다. 배정표 100편·인물 시트·시행 순서는 [2026-09-26-dialogue-scene-allocation.md](./2026-09-26-dialogue-scene-allocation.md). 3묶음은 미시작 2묶음(09-30~10-09)을 교체한다.
- **대화 흐름 검토에서 잡힌 규칙 (2026-09-20, 9판 6~10편 검토)**: (1) `Let me think about it.` 같은 응답형 문장은 앞 줄이 제안이나 질문이어야 한다. 앞 줄이 소식 전달이면 질문(`Any ideas?`)을 붙여 답이 되게 한다. (2) "왜 맨날 그래?" 처럼 습관을 단정하는 상대 대사는 일기에 그날 한 번만 있으면 창작이다. (3) 사과·책임 대사(`I should have booked.`)도 사실 게이트를 받는다. 일기에 있는 사실(15분 지각)로 바꾼다. (4) 같은 패턴은 한 장면 안에서는 금지, 열 편 안에서는 3회까지 허용(`Let me` 처럼 모두영어 빈도가 높은 것). (5) 상대가 남성 친구(정경섭)인 장면은 화자 이름만 바꾸면 되고 규칙은 같다.
- **대화 저작 규칙 (2026-09-19, 8판까지 걸려 확정)**: 상황은 누구나 겪는 기본 상황만(공항 픽업·식당·고양이 밥·운동·장보기 등), 첫 줄이 상황을 말해 장면 설명 없이도 대화만으로 맥락이 서야 하고, 지오 대사는 복습 때 외워 다시 쓸 문장만, 사실은 `~/apps/tmp/2026-09-19-diary-facts.md` 뱅크에 있는 것만, 장면 하나 = 일기 한 편, 영상 30패턴은 할당량이 아니라 우선순위. 상세는 메모리 `feedback-study-dialogue-source-bank`.

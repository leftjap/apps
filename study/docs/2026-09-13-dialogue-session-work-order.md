...
- **문장 공급원 우선순위 (2026-09-19 사용자 확정)**: 카드 문장은 **① 영상 30패턴(`FzW2gVf7SCw`) → ② 모두영어 529문장 → ③ 233 뱅크** 순으로 고른다. 지금 단계에서는 이 셋을 넘어가지 않는다. 맥락이 부자연스러울 때만 예외인데, 그 단서로 빠져나가면 안 된다(8판은 지오 카드 20장 중 모두영어 문장이 0이라 이 규칙 위반 — 다음 세션이 다시 짠다).

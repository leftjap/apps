...
## 반응형·버튼 전수 확인 (2026-09-14, "사이드바만 고정" 지적 후속)

### 프레임 도입

시안 12a 의 1280 컨테이너는 목업 프레임이 아니라 **화면 전체**다(`aside 250 + main 1028`, `stage flex:1 1 auto`, `panel 400 고정`). 앞선 수정은 대화에 548 상한만 두고 사이드바를 화면 왼쪽 끝에 남겨, 1920 에서 사이드바와 대화가 **341px** 벌어졌다. 시안에서 그 간격은 본문 padding **28px** 뿐이다.

사이드바까지 한 프레임(`.vs-frame`, `max-width:1278px = 250 + 1028`)으로 묶고 `.vs{justify-content:center}` 로 프레임째 가운데 둔다. 프레임 안에서는 시안 규칙 그대로 대화가 남는 폭을 먹는다(`.vs-stagewrap{flex:1 1 auto}`).

| 폭 | 사이드바 | 대화 | 우측 | 사이드바↔대화 | 방향 |
|---|---|---|---|---|---|
| 1920 | 250 | 548 | 400 | 28 | 가로 |
| 1440 | 250 | 548 | 400 | 28 | 가로 |
| 1280 | 250 | 548 | 400 | 28 | 가로 |
| 1200 | 250 | 470 | 400 | 28 | 가로 |
| 1100 | 250 | 779 | 779 | 28 | 세로 |
| 1024 | 250 | 703 | 703 | 28 | 세로 |
| 900 (tablet) | — | `.m-pad` 560 가운데 | — | — | 폰 레이아웃 |
| 390 | — | 줄 362 | — | — | 폰 |

1024~1100 세로 적층은 종전에 `align-items:center` + `max-width:548` 이라 사이드바 옆이 106~144px 비었다. 본문이 남는 폭을 채우게 고쳤다.

### 폰 하단 CTA 고정

작업지시서 §2-3 은 "하단 CTA(sticky)", 시안 설명문은 "폰의 상단 바와 '다음 표현' 버튼은 실제 앱에서 화면에 고정" 이라고 적었는데 기존 `.m-cta` 에는 `position` 이 없어 스크롤해야 보였다. 신규 세션 전용 `.m-cta-fixed{position:sticky;bottom:0;z-index:6}` 를 붙였다(복습·수학·요약이 쓰는 기본 `.m-cta` 는 그대로). 스크롤 0·600·900 에서 상단 바 `top:0`, CTA `bottom:844` 고정을 실측했다.

### 버튼 전수 확인

앞서 누르지 않았던 것까지 전부 눌렀다.

- **듣기 원**: `vs-cir playing eqq` + 아이콘이 이퀄라이저로 교체 → 재생 끝나면 원복.
- **전체 듣기**: 버튼 `playing` + 라벨 '재생 중' + 줄 하이라이트 1개 → 다시 누르면 `studySpeech.cancel` 호출 + 라벨·하이라이트 원복.
- **따라 말하기**: `recing` + 라벨 '녹음 멈추기' + `pri` 제거 → 채점 후 '다시 말하기'.
- **상대 줄 녹음**: 버튼과 줄 둘 다 `recing`(코랄 배경).
- **응용 듣기 원**: `playing eqq` → 원복.
- **해설**: 열기 → `open` + 섹션 노출, 닫기 → `display:none`.
- **홈으로**(데스크톱·폰): `location.hash = '#/home'`.
- **세션 종료**: 실제 `showEndConfirm` 다이얼로그("학습을 종료할까요?" / 계속하기 / 종료). 계속하기 → 닫힘·미확정, 종료 → `onConfirm`.
- 그 밖에 세그먼트 4칸, 문장 목록 4개, 접힌 카드 줄 녹음(이동 + 자동 본 녹음), 응용 녹음·카운터, 폰 대화 접기/펼치기(접힘 상태 상대 줄 녹음이 원래 줄 index 로 기록), 하단 CTA 까지 확인.

전체 테스트 81파일 **1679개** 통과, `pnpm build` 성공.


---

## 상태 귀속 결함 다섯 건 (2026-09-14, 사용자 실기기 스크린샷 후속)

사용자가 스크린샷 3장으로 잡아낸 것: **같은 대화 줄인데 1번 카드를 고른 채 녹음하면 70, 2번 카드를 고른 채 녹음하면 73·75 로 따로 기록**됐다. 그 하나를 파고들어 같은 유형을 넷 더 찾았다. 전부 "카드를 옮겼다 돌아와야 드러나는" 결함이라, 버튼을 한 번씩만 눌러 본 앞선 검증이 놓쳤다.

| # | 증상 | 원인 | 수정 |
|---|---|---|---|
| 1 | 같은 상대 줄 점수가 선택 카드마다 갈림 | `onMiniScore` 가 `exLog[현재 카드].mini[줄]` 에 저장, 화면도 현재 카드 것만 읽음 | 묶음 첫 카드(`dlgOwnerId`)로 저장 통일, 읽기는 묶음 안 모든 카드를 합쳐(`miniScoresAt`) 흩어진 옛 기록도 보이게 |
| 2 | 상대 줄만 녹음해도 필이 '다시 말하기' | 라벨이 `recLog[id].count`(본·응용·상대·체이닝 합산)를 봄 | `state.mainRecLog[cardId]` — 본 녹음 전용 카운터 신설. `exLog.utter` 는 수화로 어제 점수까지 담겨 못 씀 |
| 3 | 새로고침하면 그 라벨이 초기화 | 스냅샷에 저장만 하고 `restoreFromSnapshot` 매핑을 빠뜨림 | `activeSession.js` 에 `mainRecLog` 복원 추가 |
| 4 | 녹음해도 좌측 목록 진행이 안 뜸 (카드를 옮겨야 반영) | 목록이 렌더 1회로 굳음 | `navHost` 로 감싸 `refreshDots` 마다 다시 그림 |
| 5 | 응용·상대 줄을 녹음해도 링 캡션이 '방금 점수' | `lastScoreLive` 를 안 내리고, 내린 뒤에도 캡션 DOM 을 안 고침 | 두 경로에서 `lastScoreLive=false`, `refreshDots` 에서 캡션 글자만 갱신(링은 그대로 — 매번 `v-settle` 이 튀지 않게) |

덤으로 좌측 목록이 **현재 선택된 카드만** 진행을 숨기고 있었다. 시안 12a 를 다시 재 보니 `sel` 항목에 '말하기 2회' 가 있어(실측) 그 조건을 없앴다.

### 통합 확인 (dev, personal 시드, demo 채점)

| 단계 | 필 | 링 | 캡션 | 본 점수 열 | 첫 줄 흔적 | 목록 |
|---|---|---|---|---|---|---|
| 시작 | 따라 말하기 | — | — | 숨김 | 0 | — |
| 상대 줄 녹음 | 따라 말하기 | — | — | 숨김 | 1 | — |
| 본 녹음 | 다시 말하기 | 88 | 방금 점수 | 1개 | 1 | 말하기 1회 |
| 응용 녹음 | 다시 말하기 | 88 | 지난 점수 | 1개 | 1 | 말하기 1회 · 응용 1/6 |
| 본 녹음 재차 | 다시 말하기 | 91 | 방금 점수 | 2개 | 1 | 말하기 2회 · 응용 1/6 |
| 2번 카드 | 따라 말하기 | — | — | 숨김 | 1 | (1번 항목 유지) |
| 1번 복귀 | 다시 말하기 | 91 | 지난 점수 | 2개 | 1 | 말하기 2회 · 응용 1/6 |

2번 카드에서 상대 줄을 또 녹음하면 첫 줄 흔적이 2로 누적되고, 1번으로 돌아와도 2가 유지된다. 저장은 `exLog[대표 카드].mini` 한 곳, `#mini#` 영속 키도 대표 카드 id 로 고정된다.

전체 테스트 81파일 **1696개** 통과.


### 후속: 내가 만든 회귀 하나 (수화)

대화 줄 저장을 대표 카드로 모은 뒤, **복습에서 그 줄 점수가 사라지는 회귀**가 남아 있었다. 수화(`loadScoreHistoryState`)가 `miniLogId(카드 id, 줄)` 로 **카드마다** 찾는데, 신규는 이제 대표 카드 id 로만 저장하기 때문이다. 복습은 카드 하나씩 보므로 대표가 아닌 카드에서는 빈 값이 된다.

수화를 **줄 텍스트** 기준으로 바꿨다 — `sentenceId` 가 `#mini#<줄 영어>` 로 끝나는 행을 모은다. 대화 줄은 카드가 아니라 대화에 속하므로 이쪽이 옳고, 2026-09-14 이전에 카드별로 흩어져 저장된 기록도 같은 줄로 합쳐진다. 본 점수(카드 id 정확 일치)와 드릴(`#drill#`)은 종전대로 카드별이다(회귀 테스트 추가).

---

## 실제 녹음·실기기 계열 검증 (2026-09-17, "미검증을 전부 검증하라" 지시 후속)

앞선 확인은 `demo=1` 의 가짜 채점과 데스크톱 브라우저까지였다. 여기서는 마이크에서 Azure 채점까지 실제로 태우고, iOS 계열 엔진에서 폰 레이아웃을 실측했다.

### 1) 가짜 마이크 E2E (Chrome, dev 5183, 봇 계정)

`study-fake-mic-e2e` 스킬 절차 그대로 진행했다. Azure TTS 로 `[무음 2초][문장][무음 9초]` WAV 두 개를 합성해
(`We landed early. It's 4:20.` / `I'm on my way.`) `--use-file-for-fake-audio-capture` 로 넣었고,
봇 계정(`claude-bot@today.local`)에 `bot-e2e-` 카드 4장을 심어 세션을 열었다.

| 확인 | 결과 |
|---|---|
| 상대 줄 실제 녹음 | Azure 채점 **94** — 줄 흔적에 표시 |
| 네 카드를 옮겨 다니며 같은 줄 | 어디서 보든 `firstTrace:["94"]` (결함 1 수정 확인) |
| 본 문장 실제 녹음 | **99** · 필 '다시 말하기' · 링 99 · 캡션 '방금 점수' · 본 점수 열 1개 · 목록 '말하기 1회' |
| 상대 줄 재녹음 | `firstTrace:["94","95"]` 누적 |
| 브라우저 완전 재시작 | 복원 후에도 `firstTrace:["94","95"]`, 라벨 유지 (결함 3 수정 확인) |
| 복습 화면 | 대화 줄에 `94`·`95` 가 그대로 — 수화 회귀 없음 |

서버 로그로도 대조했다. `study_pronunciation_log` 의 `sentence_id` 가
...
| scrollTop | `.m-topb` top / bottom | `.m-cta-fixed` top / bottom | 뷰포트 |
|---|---|---|---|
| 0 | 0 / 53 | 558 / 635 | 635 |
| 757 (중간) | 0 / 53 | 558 / 635 | 635 |
| 1514 (끝) | 0 / 53 | 558 / 635 | 635 |

대화를 접은 상태(스크롤 범위 885)에서도 세 지점 모두 같은 값이었다. 즉 스크롤 전 구간에서 두 막대가 붙어 있다.

### 2-1) 실제 iOS 26.5 시뮬레이터에서 스크롤한 화면

시뮬레이터에 터치를 넣을 수 없어 우회했다. `ios-webkit-debug-proxy`(1.9.2) 를 깔아 시뮬레이터 Safari 에
붙으려 했지만 `com.apple.webinspectord_sim` 소켓을 어디서도 찾지 못해 기기 목록이 빈 채로 나왔다
(`/json` 이 `[]`, 디버그 로그도 기기 탐색 시도 없이 끝난다). 시뮬레이터 쪽 `WebInspectorEnabled` 를 켜고
Safari 를 다시 띄워도 같았다. Xcode 26 탓인지 다른 조건 탓인지는 확인하지 못했다. 대신 **WebKit 으로 렌더한 세션 화면의 DOM 을 통째로 뜨고 스크립트만 제거해** 정적
페이지 세 벌(맨 위·중간·맨 아래)로 만든 뒤, 로컬 서버에 올려 `simctl openurl` 로 열었다. 로드 직후
`window.scrollTo` 로 위치를 잡으므로 터치가 필요 없다.

세 장 모두에서 상단 바는 화면 맨 위에, 하단 CTA 는 Safari 주소창 바로 위에 붙어 있었다.
실기기 계열에서 `position:sticky` 가 스크롤 전 구간 유지되는 것을 눈으로도 확인한 셈이다.

막대 뒤 비침은 두 환경이 다르다. `.m-topb` 배경은 `oklch(97.5% .009 95/.92)` 로 92% 불투명이고
`backdrop-filter:blur(8px)` 가 걸려 있다. 눈이 아니라 픽셀로 쟀다 — 같은 화면을 스크롤 0 과 중간에서
찍어 상단 바 영역(1125×145)만 빼 보면:

| | 최대 차이 | 평균 | 차이 5 초과 픽셀 |
|---|---|---|---|
| 시뮬레이터 Safari | 2 / 255 | 0.158 | **0 개** |
| 헤드리스 WebKit | 16 / 255 | 0.381 | 5,584 개 |
| (대조군) 본문 영역 | 201 / 255 | 14.332 | 27,741 개 |

시뮬레이터에서는 뒤 내용이 바뀌어도 상단 바 픽셀이 사실상 그대로다(0.8% 이내, 눈으로 식별 불가).
헤드리스에서만 6% 수준으로 비쳐 "you sleep?" 이 읽힌다. 92% 불투명이면 이론상 최대 8% 까지 비치므로,
오히려 헤드리스가 명세에 가깝고 iOS Safari 는 blur 로 더 뭉갠다. 어느 쪽이든 **앱 화면의 결함이 아니고,
헤드리스 스크린샷으로 색·투명도를 판정하면 안 된다.**

### 3) 폰 버튼 전수 (같은 WebKit·같은 폰 크기, 두 번에 나눠 실행, 소리는 음소거)

| 버튼 | 결과 |
|---|---|
| 대화 접기 → 펼치기 | 줄 8 → 2 → 8, 라벨도 함께 바뀜 |
| 다른 카드 줄 클릭 | 1/4 → 2/4, 세그먼트 `f---` → `ff--`, 선택 줄 이동 |
| 상대 줄(카드 아님) 클릭 | 선택 그대로 — 점프 대상이 아님 |
| 선택 줄 듣기 필 | 필에 `playing` 이 붙고 선택·진행은 그대로 (필 클릭이 `onJump` 를 부르지 않음) |
| 전체 듣기 | 헤더 '재생 중', 첫 줄에 `playing` |
| 표현 해설 펼치기/접기 | `open` 클래스와 본문 `display` 가 함께 토글 |
| 상단 진행 세그먼트 3 | 1/4 → 3/4, 선택 줄이 3번 카드로 |
| 다음 표현 | 2/4 → 3/4 |
| 홈으로 | `#/home`, 세션 화면 해제 |
...
봇 계정의 `bot-e2e-` 카드 4장(`study_today_lessons`·`study_review_queue`)과 봇 `study_pronunciation_log` 4행을
지웠다. 검증 중 복습 화면에 끼어들지 않게 2026-12-31 로 밀어 두었던 봇 옛 카드 2장의 `next_review` 는
등록 규칙(세션 날짜 + 1일)대로 2026-09-14·2026-09-16 으로 되돌렸다. 봇 토큰이 들어 있던 임시 파일
(`e2e-sess.json`·`e2e-login.html`·`~/apps/tmp/e2e-20260917/`)과 전용 크롬 프로필도 지웠다.
봇 계정의 `study_daily_stats` 는 이전 세션들 것과 함께 남겨 두었다(봇 계정 통계라 사용자 화면과 무관).

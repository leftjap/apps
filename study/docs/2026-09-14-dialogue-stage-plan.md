...
시뮬레이터에 터치를 넣을 수 없어 우회했다. `ios-webkit-debug-proxy` 를 깔아 시뮬레이터 Safari 에 붙으려
했지만 Xcode 26 에서는 `com.apple.webinspectord_sim` 소켓이 잡히지 않아 기기 목록이 빈 채로 나왔다
(`/json` 이 `[]`). 대신 **WebKit 으로 렌더한 세션 화면의 DOM 을 통째로 뜨고 스크립트만 제거해** 정적
...
`backdrop-filter:blur(8px)` 가 걸려 있다. **실제 시뮬레이터 Safari 에서는 뒤 글자가 전혀 비치지 않는다**
(상단 바·하단 CTA 를 3배 해상도로 잘라 확대해 확인). 헤드리스 WebKit 스크린샷에서만 "you sleep?" 같은
글자가 또렷하게 비친다. 헤드리스도 `CSS.supports` 는 true 고 단순 확인 페이지에서는 blur 가 보이지만,
이 화면에서는 실기기만큼 흐려지지 않는다. 즉 **헤드리스 쪽 렌더 차이이고 앱 화면의 결함이 아니다.**

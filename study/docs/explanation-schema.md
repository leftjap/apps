...
```js
{
  ...
  drills: [
    ...
    // 각 문장 = 듣기(TTS)/녹음(발음채점) 가능. 개수는 품질 통과분만 4~10 (2026-07-22), 정원 채우기 금지
  ],
}
```
...
**규칙:**
- **개수 (2026-07-22 개정)**: **4~10개 — 게이트 차단.** 정원(quota) 채우기 금지: 갯수를 맞추려 저품질 변주를 만들지 말고 **품질 통과분만** 넣는다. 표현이 생산적이면(빈출 패턴·콜로케이션 풍부) 8~10개까지, 빈약하면 4~5개. (구 "핵심 6~8/쉬움 3" 폐기 — 실측: 루틴이 전 카드 5~6 획일 저작)
...
```js
{
  ...
  drills: [
    ...
    // … 품질 통과분만 4~10 — 살아있는 문장·축 다양성 (§drills 규칙 2026-07-22)
  ],
  grammar: [

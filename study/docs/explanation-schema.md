...
| `chunks` | `[base, 증분1, …]` 2~8개. **1번째 chunk = base** (체이닝은 base 학습 완료 후 세션 — 아는 것에서 시작). **증분은 기본 1~3단어**(분절 불가한 덩어리만 4 — 초과 경고). 끊는 기준 — ① **완결 발화 지점** 최선(`…wait a little` — 거기까지만 말해도 실사용 문장) ② **예고형 말끝 허용**(`…but \|`·`…we should \|` — 실제 구어에서 말 끌며 멈추는 지점, 뒷 내용 예고로 듣기 집중↑) ③ **피할 지점**: 관사·전치사·소유격 바로 뒤(`the \|`·`in \|`·`my \|` — 말끝이 될 수 없는 곳). **순서대로 이어붙이면 `target` 과 일치**(불일치 차단). **단계 수는 규정하지 않는다** — 증분 규칙만 지키면 target 길이가 단계 수를 자연 결정 (13단어 → 6~8단계) |

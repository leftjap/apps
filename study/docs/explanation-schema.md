...
```js
{
  ...
  drills: [
    ...
    // 각 문장 = 듣기(TTS)/녹음(발음채점) 가능. 개수는 품질 통과분만 4개 이상·상한 없음 (2026-07-28), 정원 채우기 금지
  ],
}
```
...
| 필드 | 형식 | 비고 |
|---|---|---|
| drills | `[{en, ko, kr}]` 객체 배열 | 각 행 = 응용 문장 1개. en 필수, **ko 의무**(게이트 차단 — 세션 '생산 연습(한→영)'이 ko 를 문제로 씀), **kr = 한글 음차 의무 (en 신규, 2026-06-10)** — 녹음 연습 발음 가이드, **연음 경계 재분절 표기** (guide-en §7: 캔 유 X → 캐뉴 O) |

**규칙:**
- **개수 (2026-07-28 개정)**: **4개 이상 — 하한만 게이트 차단, 상한 없음 (>14 는 경고).** 정원(quota) 채우기 금지 원칙은 유지: 갯수를 맞추려 저품질 변주를 만들지 말고 **품질 통과분만** 넣는다. 표현이 생산적이면(빈출 패턴·콜로케이션 풍부) 10개를 넘어도 좋다 — 개수는 결과이지 목표가 아니다(사용자 지시 "필요한 만큼"). 빈약하면 4~5개. (구 상한 10 차단 폐지 — 상한이 '정원 채우기 금지' 취지와 모순: 활용도 높은 표현의 연습을 막았음. 그 전 "핵심 6~8/쉬움 3" 도 폐기 이력)
- **살아있는 문장 의무 (2026-07-22)**: 각 드릴은 **원어민이 그대로 말할 법한 완결 발화**여야 한다. 해당 표현의 **고빈도 콜로케이션 프레임을 우선** 사용 (come over → ~ for dinner / ~ tonight / ~ here). 조합이 자연스러운지 확신이 없으면 **웹에서 실사용을 확인**하고 채택한다. 근거: 정형 표현(formulaic sequences) 명시 연습이 발화 유창성을 높인다는 연구 방향 — 기계 조립문은 이 효과가 없다.
- **축 다양성 의무 (2026-07-22)**: 한 카드의 드릴 세트는 변주 축(주어·시제·극성·문형·목적어/상황) 중 **3개 이상**을 커버한다. 게이트 근사 검사 = 첫 단어 3종 이상 (전 드릴이 같은 주어로 시작하면 차단).
- **수준·정렬 (2026-07-22)**: 새 어휘는 드릴당 최대 1개(i+1). 세트는 **짧고 쉬운 것 → 길고 복합적인 것** 순으로 정렬 (재생 속도가 길이 기준이라 학습 곡선과 맞물린다: ≤6단어 빠르게 / 10단어+ 보통).
...
- ⛔ **근접중복 금지 (게이트 차단, 2026-07-10)**: **base 의 문법 맥락이 그대로면 변주가 아니다.**
  ...
  3. **원문 인용형 (2026-07-22 신설, 게이트 `quotedClipDrill`)** — 여러 문장짜리 드릴에서 한 문장이 base 그대로인 것 (에피소드 대본 인용·스터터 포함). 그건 변주가 아니라 **체이닝(chain)의 몫**이다.
     ❌ `I know. I'll be there as soon as I can.` · `I.. I don't know what to do. What do I do?` · `What's going on? Something's going on.`
     실측(2026-07-22): 최근 9개 시드 354드릴 중 10건이 이 유형으로 게이트를 통과하고 있었다.
...
"chain": {
  ...
  "chunks": ["What did I do", "wrong", "this time?", "Tell me", "honestly,", "please."],
  "ko": "내가 이번엔 뭘 잘못했는데? 솔직하게 말해 줘, 제발."
}
```
(단계 4→5→7→9→10→11단어, 6단계 — 1단계 = base, 이후 증분 1~2단어씩. **모든 중간 단계가 그 자체로 완결된 실사용 문장**)

| 필드 | 규칙 |
|---|---|
| `target` | **base 로 시작하는 보편·고활용 프레임**으로 확장한 실제 쓰는 발화 — 아무 확장 금지(빈도·활용도 우선, 코퍼스 기반 청크 선정 원칙). 자연스럽게 확장되는 만큼만 — 11~15단어면 단계 5~7개가 자연 발생, 안 늘어나는 표현은 짧게(8단어 미만 경고). **억지 확장 금지** |
| `chunks` | `[base, 증분1, …]` 2~8개. **1번째 chunk = base** (체이닝은 base 학습 완료 후 세션 — 아는 것에서 시작). **증분은 기본 1~3단어**(분절 불가한 덩어리만 4 — 초과 경고). 끊는 기준 — ① **완결 발화 지점** 최선(`…wait a little` — 거기까지만 말해도 실사용 문장) ② **예고형 말끝 허용** — **연결어**(접속사 `but`/`or`/`because`·조동사구 `we should`·담화표지 `say`·다음절 전치사 `without`/`after`) 뒤: 실제 구어에서 말 끌며 멈추는 지점이고 뒷 내용 예고로 듣기 집중↑ ③ **피할 지점** — **명사구를 열다 마는 것**(관사 `a`/`the`·소유격 `my`/`your`·단음절 전치사 `in`/`of`/`at`) 뒤: 말끝이 될 수 없는 곳. **순서대로 이어붙이면 `target` 과 일치**(불일치 차단). **단계 수는 규정하지 않는다** — 증분 규칙만 지키면 target 길이가 단계 수를 자연 결정 (13단어 → 6~8단계) |
| `ko` | **의무**. 마지막 단계 3회 실패 시 첫 힌트 |

- **단계 세분화 (2026-07-13 사용자 결정, 최종)**: 체이닝은 **base 학습(따라말하기·응용)이 끝난 뒤** 나오는 세션 — 학습자가 이미 아는 base 에서 시작해 모르는 확장을 붙인다. 목적 = ① **base 다회 발화** (매 단계 base 부터 재발화 — 반복→유창성, 4/3/2 연구) ② **듣기 집중** ③ **확장 감각** (부사·형용사·접속절이 붙으며 문장이 자라는 모습을 단계마다 목격). 그래서 증분은 잘게(1~3단어, 단계 5~8개) — 완결 발화 지점이 최선이지만 **예고형 말끝(`…but`)도 허용**한다(2026-07-13 사용자 지적: 실제 구어의 멈춤 지점이고 뒷 내용 예고가 듣기 집중을 높인다. sentence combining 연구·억양 단위 1~4단어 근거 유지). ⚠️ 역순(backchaining·꼬리부터)은 검토 후 **폐기** — base 를 이미 아는 학습자에게 아는 것부터 시작하는 전진 확장이 맞다(사용자 결정).
- **인위 제약 금지 (2026-07-13 사용자 결정)**: 단계 **수**·목표 길이를 채우려는 억지 확장·억지 분절이 세분화보다 해롭다. chain 게이트 경고(증분 과대)는 **자연스러움을 해치면서까지 해소하지 말 것** — 재구성이 부자연스러우면 경고를 남기고 사유를 보고에 적는다.
...
- **단계 행 점수 원 + 기록 (2026-09-03, 사용자 지시 "발화한 점수를 빠뜨리지 말 것")**: 체이닝·생산 연습 발화도 감점제 점수를 행에 원으로 그리고(최근 8개), `pronunciationLog` 에 `<카드id>#chain#<단계 문장>` · `<카드id>#prod#<드릴 문장>` 키로 저장한다(`chainLogId`·`prodLogId`). 종전엔 '오늘 발화'에만 세고 점수는 어디에도 남지 않았다. 통과 ✓(커버리지)와 점수 원은 별개 표시. 메인 문장 점수 원 상한은 같은 날 5→10.
...
## miniDialogue — 미니대화 (2026-09-08 신설, 작업지시서 §1~§4 · 선택 필드)

타깃 표현이 **어떤 상대 발화와 상황 뒤에 나오는지** 짧게 듣게 하는 contextual input. `situation` 이 설명문이라면 miniDialogue 는 그것을 실제 영어 입력으로 바꾼 것이다. **암기·진행 조건 대상이 아니다** — 화면(신규 세션 문장 카드 **아래**, `sessionExprV2.js` `miniDialogueEl`)은 전체 듣기·한 줄 듣기·타깃 줄 강조에 더해 줄마다 녹음·채점(응용 행과 같은 점수 원·`#mini#` 이력, 2026-09-12 사용자 결정으로 2026-09-08 의 '듣기 전용'을 뒤집음)이 있다. `situation` 이 있으면 라벨 아래 장면 한 줄로 보여준다. 통과 판정·다음 잠금·SRS·완료 표시는 없다. 복습 세션(`sessionReviewV2.js`)에서는 정답 공개 뒤에만 같은 블록을 카드 아래에 생성한다. 코어100 100장 전부에 적용(2026-09-08). 복습 렌더러·공유 해설 패널에는 넣지 않는다(정답 유출).

| 필드 | 형식 | 비고 |
|---|---|---|
| miniDialogue | `[{speaker, en, ko?, kr?, name?}]` 2~8턴 | `speaker` 는 `A`(여성)/`B`(남성) — 성별로만 나눈다. 재생 목소리는 같은 성별 안에서 누를 때마다 바뀐다(A: Ava·Emma, B: Andrew·Guy·Eric, 2026-09-25). `name` 은 화면 화자 칸에 표시할 이름(선택, 없으면 글자). `kr` 은 한글 발음(줄 부제 `kr · ko`, 응용 행과 같은 구성). 타깃 줄의 `en` 은 카드 `sentence` 와 **문자열 완전 일치**, 정확히 1회. `ko`·`kr` 은 일반 트랙 선택, **personal 트랙 의무** |

**규칙** (검사기 `validate-seed.mjs`, 형식 검사이지 학습 게이트가 아님):
- 차단: 2~8턴 밖(2026-09-12, 종전 2~4) / 타깃 줄이 정확히 1개가 아님 / 타깃 표현이 다른 줄에도 나옴 / 타깃 외 줄 13단어 이상 / **아직 안 배운 뒤쪽 묶음**의 코어100 핵심 표현이 다른 줄에 들어감(새 학습 부담).
- 경고: 타깃 외 줄 11~12단어(10단어 이하 권장이지만 자연스러움을 위해 하드 조건은 아님) / 이미 배운 앞쪽 묶음 표현이 상대 발화에 나옴(복습 효과라 허용).
- 사람 검수: 대화 전체 난도가 타깃보다 높지 않은가, 상대 발화가 타깃을 **유발**하는가(상황을 영어로 옮긴 수준이면 실패), 카드마다 패턴이 획일적이지 않은가(`A 질문 → B 타깃 → A Okay` 금지). 턴 수는 표현에 따라 2~8(personal 트랙은 대화 한 편 6~8), 코어100 은 평균 3.
- 주변 줄은 A2~B1, 새 문법·고급 표현 금지, 별도 암기 대상 아님. 타깃은 대개 2번째 줄이지만 상대가 말하는 문장이 타깃인 카드(입국심사 질문 등)는 1번째 줄 허용.
...
```js
{
  ...
  drills: [
    ...
    // … 품질 통과분만 4개 이상·상한 없음 — 살아있는 문장·축 다양성 (§drills 규칙 2026-07-28)
  ],
  grammar: [

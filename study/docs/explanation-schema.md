...
타깃 표현이 **어떤 상대 발화와 상황 뒤에 나오는지** 짧게 듣게 하는 contextual input. `situation` 이 설명문이라면 miniDialogue 는 그것을 실제 영어 입력으로 바꾼 것이다. **평가·암기 대상이 아니다** — 화면(신규 세션 문장 카드 **아래**, `sessionExprV2.js` `miniDialogueEl`)은 전체 듣기·한 줄 듣기·타깃 줄 강조뿐이다(줄마다 듣기 + **녹음** — 2026-09-12 사용자 결정으로 2026-09-08 의 '듣기 전용'을 뒤집음. 녹음은 응용 행과 같은 채점·점수 원·오늘 발화 집계·`#mini#` 이력이고 진행 조건·SRS 영향은 없다. `situation` 이 있으면 라벨 아래 장면 한 줄로 보여준다). 통과 판정·다음 잠금·SRS·완료 표시는 없다. 복습 세션(`sessionReviewV2.js`)에서는 정답 공개 뒤에만 같은 블록을 카드 아래에 생성한다. 코어100 100장 전부에 적용(2026-09-08). 복습 렌더러·공유 해설 패널에는 넣지 않는다(정답 유출).

...
타깃 표현이 **어떤 상대 발화와 상황 뒤에 나오는지** 짧게 듣게 하는 contextual input. `situation` 이 설명문이라면 miniDialogue 는 그것을 실제 영어 입력으로 바꾼 것이다. **평가·암기 대상이 아니다** — 화면(신규 세션 카드 위, `sessionExprV2.js` `miniDialogueEl`)은 전체 듣기·한 줄 듣기·타깃 줄 강조뿐이고 녹음·통과 판정·다음 잠금·SRS 가 없다. 복습 렌더러·공유 해설 패널에는 넣지 않는다(정답 유출).

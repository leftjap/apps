...
| 필드 | 규칙 |
|---|---|
| `target` | base 에 **확장 성분을 하나씩 쌓아** 지은 실제 쓰는 발화. **자연스럽게 확장되는 만큼만** — 보통 8~14단어·2문장 수준(8단어 미만 경고 — 짧으면 앵무새 반복). 길게 안 늘어나는 표현은 8~9단어로 둔다. **억지 확장 금지** (실회화 평균 발화 ~9단어, BNC) |
| `chunks` | `[base, 증분1, …]` 2~8개. **증분은 기본 1~3단어**(분절 불가한 절만 4 — 초과 경고). 끊는 지점 = **자연 휴지점**(끊어 읽어도 유창하게 들리는 곳): 절·구 경계, 문장부호 뒤, 동사구 완결 뒤+부사 앞(`Tell me \| honestly`), 겹층 부사구 사이(`at least \| a little`). **금지**: 기능어 뒤 끊기(`… before \|`)·목적어 스트랜딩(`you judge \| me`) — 절 내부 휴지는 비유창성 지표. **순서대로 이어붙이면 `target` 과 일치**(불일치 차단). **단계 수는 규정하지 않는다** — 증분 규칙만 지키면 target 길이가 단계 수를 자연 결정 |
| `ko` | **의무**. 마지막 단계 3회 실패 시 첫 힌트 |

- **단계 세분화 (2026-07-13 사용자 결정, 같은 날 재정의)**: 체이닝의 목적 = ① **base 를 많이 말하게** (전진 누적이라 매 단계 base 부터 재발화 — 반복이 유창성을 올린다, 4/3/2 연구) ② **듣기 집중** ③ 확장 요소(부사·형용사·접속사)가 붙는 감각. 그래서 증분은 잘게(기본 1~3단어) — 단 끊는 지점은 **자연 휴지점만**: 절 내부 휴지는 비유창성의 최강 지표(휴지 위치 연구)라, 기능어 뒤 끊기·목적어 스트랜딩은 단계 오디오가 나쁜 끊어읽기를 모델링하게 된다. 세분이 안 되는 절(예: `before you judge me`)은 통짜 4단어로 두고, 부족하면 **target 자체를 증분 스택으로 재구성**한다 (sentence combining 연구·억양 단위 1~4단어 근거는 유지).

...
| `chunks` | `[base, 증분1, …]` 2~8개. **1번째 chunk = base** (체이닝은 base 학습 완료 후 세션 — 아는 것에서 시작). **증분은 기본 1~3단어**(분절 불가한 덩어리만 4 — 초과 경고). 끊는 기준 = **누적 접두부가 그 자체로 완결 발화가 되는 지점**(거기까지만 말해도 실제 쓸 수 있는 문장 — `What did I do \| wrong \| this time?` 각 단계 전부 실사용 문장). **금지**: 완결 발화가 안 되는 지점 — 접속사·전치사 뒤(`…before \|`)·목적어 스트랜딩(`you judge \| me`). **순서대로 이어붙이면 `target` 과 일치**(불일치 차단). **단계 수는 규정하지 않는다** — 증분 규칙만 지키면 target 길이가 단계 수를 자연 결정 |
| `ko` | **의무**. 마지막 단계 3회 실패 시 첫 힌트 |

- **단계 세분화 (2026-07-13 사용자 결정, 최종)**: 체이닝은 **base 학습(따라말하기·응용)이 끝난 뒤** 나오는 세션 — 학습자가 이미 아는 base 에서 시작해 모르는 확장을 붙인다. 목적 = ① **base 다회 발화** (매 단계 base 부터 재발화 — 반복→유창성, 4/3/2 연구) ② **듣기 집중** ③ **확장 감각** (부사·형용사·접속절이 붙으며 문장이 자라는 모습을 단계마다 목격). 그래서 증분은 잘게(1~3단어, 단계 5~7개), 각 중간 단계는 **그 자체로 완결된 실사용 문장**이어야 한다 — 어색한 스텝 오디오가 원천 차단되고, 중간 단계 문장도 그대로 써먹을 수 있다 (sentence combining 연구·억양 단위 1~4단어 근거 유지). ⚠️ 역순(backchaining·꼬리부터)은 검토 후 **폐기** — base 를 이미 아는 학습자에게 아는 것부터 시작하는 전진 확장이 맞다(사용자 결정).

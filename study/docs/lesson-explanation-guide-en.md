...
| `drills` | `[{en, ko, kr}]` 4~10개 (2026-07-22 — 정원 채우기 금지, 품질 통과분만) — **kr = 한글 음차 의무** (녹음 연습 발음 가이드 — 작성 규칙은 §7 "phonetic_kr 의 정의"). 개수·구성·살아있는 문장·축 다양성은 [explanation-schema.md](./explanation-schema.md) §drills 규칙. ⛔ **근접중복·원문 인용형 금지** — 게이트가 차단 |
...
### 6.3-미니대화 저작 규칙 (2026-09-08, 코어100 표현 카드 선택 필드 `miniDialogue`)

- 목적은 **맥락 입력**이다. 타깃 표현 하나가 어떤 상대 발화·상황 뒤에 나오는지 2~8턴으로 보여 준다(personal 트랙은 대화 한 편 6~8턴). 대화 전체를 외우게 하지 않는다.
- 타깃 줄 = 카드 `sentence` 그대로(완전 일치, 1회). 주변 줄은 A2~B1, 10단어 이하 권장(11~12 는 경고), 새 문법·고급 표현·아직 안 배운 코어100 표현 금지(검사기 차단).
- `situation` 을 영어로 옮기지 말고, **상대 발화가 타깃을 유발**하게 쓴다. 턴 수와 타깃 위치를 카드마다 다르게(2~8턴, 코어100 평균 3).
- personal 트랙(2026-09-12)은 줄마다 `en/ko/kr` 의무, `name` 선택, 줄마다 듣기·녹음. 정본은 `explanation-schema.md §miniDialogue`.
- personal 트랙 저작 규칙(2026-09-26 개정, 상세는 `2026-09-13-dialogue-session-work-order.md §7`, 장면 배정표·인물 시트는 `2026-09-26-dialogue-scene-allocation.md`): 장면은 교재형 일반 상황(공항·호텔·주문·길 묻기·전화 예약·병원 접수 등)을 인물 시트에 맞게 창작하고, 실제 인물·동물 사실만 시트와 일치시킨다. 첫 줄이 상황을 말해 `situation` 없이도 대화만으로 맥락이 서야 하고, 한 가닥 대화를 먼저 쓴 뒤 줄마다 바로 앞 줄에 답하는지 확인한다. 지오 줄(카드 문장)은 상황을 말하는 문장이어야 하고(반응형은 편당 1~2줄), 공급원 순위는 ① 영상 30패턴 → ② 모두영어 그대로 → ③ 233 뱅크 — 재사용은 뜻이 같을 때의 선택 기준이지 기본값이 아니다. 같은 패턴은 한 장면 안에서 두 카드에 쓰지 않는다. 검증은 `validate-seed`(형식) + `scripts/dialogue/verify-draft.mjs`(근거 게이트) + `scripts/dialogue/check-sources.mjs`(공급원 대조).
- 형식 정본과 검사 등급은 [explanation-schema.md §miniDialogue](./explanation-schema.md). 첫 적용 = 19~24번(2026-09-06 시드), 검토 뒤 25~100 확장.
...
> `docs/core100-gold.json` 과 대조). Supabase 사용자 행도 PATCH 로 반영·화면 검증 완료(2026-09-09). 드릴 `drills[].kr` 과 `phonemes` 팁의 옛 표기는 남음 — 순서는
...
(예시 표기는 확정 gold 의 규약을 따른다 — 약형 you 는 ㅓ 계열, are 의 r 은 유지. 2026-09-09 동기화)
| 문장 | 사전 표기 (X) | phonetic_kr (O) |
|---|---|---|
| I'm not gonna lie | 아임 낫 거나 라이 | **아임 나러나 라이** |
| What are you doing? | 왓 아 유 두잉 | **워러r여 두잉?** |
| Did you eat? | 디드 유 잇 | **디저 잇?** |
| pretty rough | 프리티 러프 | **프리리 러프** |

### 실측 기록은 별도 파일로 옮겼다 (2026-09-08)

7월 재분절 의무, 8월 PA 실측, 8월 장모음 실측, 9월 재반증은 모두
[pronunciation-research-log.md](./pronunciation-research-log.md) 로 이동했다. **그 기록은 현재 명령이 아니다.**
현재 명령은 위 "phonetic_kr 의 정의" 절 하나뿐이다.

다만 그 로그에서 **현재도 유효한 규약 두 가지**와, gold 머리말에서 승격한 규약 하나를 여기에 남긴다.

- **장모음은 겹모음으로 적는다**: `mean`→미인 · `need`→니이드 · `keep`→키입 · `please`→플리이즈.
  (근거·수치는 로그 참조. 홑모음은 다른 단어로 새는 실패가 잦았다.)
- **/v/·/z/·/f/·/r/ 라틴 병기**(해v·씸z·f퍼니·모어r): 현행 유지. 한국어 TTS 가 라틴을 못 읽어
  이 문서의 실측 절차로는 우열을 가릴 수 없다. 판단 근거가 없으므로 바꾸지 않는다.
- **단어 경계는 붙여 적는다** (2026-09-13 확정 — `core100-gold.md` 머리말 규약 1 을 규칙으로 승격. 계기: personal 파일럿이
  세 항목 요약만 보고 저작돼 `테익 여r`·`낫 온` 류가 통과했고, gold 100문장 안에서도 검토자가 본 경계만 판정돼 같은 환경이 갈려
  있었다): 앞 단어가 파열음(p t k b d g)·비음(m n)·s 로 끝나고 뒤 단어가 모음으로 시작하면 한 어절로 적는다.
  `어라이v더r리`(#67 arrived early) · `커나이`(#61 can I) · `퍼r퍼서v`(#51 purpose of) · `테이커`(#79 take a) ·
  `아이멀러r직`(#73 I'm allergic) · `아이몬 마이 웨이`(personal). 어말 t 는 뒤 모음의 강세와 무관하게 flap ㄹ 로 붙인다
  (`대러겐`·`와라이`·`게로f`·`와렐스`, gold 규약 6). d·t·k 뒤에 약형 you/your 가 오면 `저·처·켜(r)`(`커저`·`왓처`·`메이켜`·`테이켜r`).
  **띄우는 자리**: ① 라틴 글자 f v z r 로 끝나는 어절(`f퍼r 어`·`데이z 어f`) ② ㄹ 로 끝나는 어절(`어니그잼플 어v`·`올 위켄드`·`스틸 엇`)
  ③ ㅇ 받침(ŋ)으로 끝나는 어절 — 옮길 초성이 없다(`게링 업`) ④ 뒤 단어가 반자음 w·y 로 시작(`웬 위`·`윌 여`·`컨 여`)
  ⑤ 문장 경계·쉼표·pause ⑥ 한글로 적은 고유명사(`소연 이z`). 음운 규칙이 아니라 표기 규약이므로 pause·phrase boundary
  판단은 문장별로 하고, `scripts/validate-seed.mjs` 의 `붙여 적기 후보` 경고는 사람이 볼 목록이지 자동 치환이 아니다(gold §1.5-①).
  gold 100문장 안에서 검토 없이 현행 그대로 남은 경계(`호움 언`#38·`디스 인`#9·`익스플레인 에v리씽`#43 등)는 이 규약으로
  재감사 대기 — 후보 목록 `~/apps/tmp/2026-09-13-gold-boundary-audit.md`.
- **gold 선례에서 확인한 관례 두 가지** (2026-09-13, 새 세션 시뮬레이션에서 어긋난 자리): ① 자음 앞 st 의 t 는 빠진다 —
  `라스 파r러v`(#8 last part) · `베스 f퍼r`(#18 best for) · `넥스 트레인`(#61 next train) → `next Friday → 넥스 f라이데이`.
  ② 강세 있는 s 는 ㅆ — `쏘리`(#1) · `쎄이`(#1) · `씨이`(#6) · `택씨`(#59 taxi). (gold 밖 적용 예: six → `씩스`.)
- **검사기** (`scripts/validate-seed.mjs` `krRuleFindings`, 2026-09-13): 코어100·personal 트랙은 gold 에 정당한 용례가 없는 옛 표기를
  **차단**한다 — 구개음화 ㅠ(`쿠쥬`), 확정 조합 불일치(`do you`≠더여·`take your`≠테이켜r 등 12종), 비강세 at 의 `앳/엣`, 단어 안 flap 옛 표기
  (`워터`·`포r티`). 문맥 판단이 필요한 것은 **경고**로 낸다 — 비강세 you 의 `유`, 라틴 병기 누락, 장모음 겹모음, I'll, 단어 경계 붙여 적기,
  자음 앞 st. 2026-09-09 이전 RealClass 장면 시드(track 없음)는 grandfather 로 경고만. 경고는 자동 치환이 아니라 사람이 볼 목록이다.

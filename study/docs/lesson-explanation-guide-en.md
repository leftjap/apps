...
| **표현 카드** **1~2장** (PPP 집중 추출) | 1~ | 장면 속 **목표적합 고빈도 표현 1~2개** — 뜻/핵심 + drills(개인화 변형) + 듣기/녹음. (구 5~7장 = 과다추출, 2026-06-29 폐기) |
...
| `drills` | `[{en, ko, kr}]` 4~10개 (2026-07-22 — 정원 채우기 금지, 품질 통과분만) — **kr = 한글 음차 의무** (녹음 연습 발음 가이드, 연음 경계 재분절 §7). 개수·구성·살아있는 문장·축 다양성은 [explanation-schema.md](./explanation-schema.md) §drills 규칙. ⛔ **근접중복·원문 인용형 금지** — 게이트가 차단 |
...
- ⚠️ **재INSERT 안전**: 시드 재적재(upsert)는 `completed=false` 로 merge → **사용자가 해당 카드 학습을 시작한 뒤에는 같은 카드 재INSERT 금지** (학습 완료 리셋됨). 보강은 학습 전에만 — 2026-09-03 부터 `seed-supabase.mjs` 가 payload id 로 서버 행을 찾아 완료된 카드를 **날짜와 무관하게** 차단한다(옛 날짜 파일 재적재로 완료가 풀리던 사고 수정)
...
### 약점 음소 가중 (단계 3-4 컨텍스트 연결 — 2026-06-10 Step 4)

spec §5-0 단계 3-4 가 조회하는 학습자 컨텍스트를 장면 선정에 반영한다:

1. `study-read-user-context.yml` 산출 JSON 의 `weak_phonemes_en` **상위 3 음소** + 최근 7일 정답률 확보
2. 발췌 기준 4종 통과 후보 장면들에 **가중 점수**: 약점 음소 포함 표현 수 × 가중 (상위 1·2·3 = 3/2/1). 약점 음소 포함 표현 ≥50% 권장 (콩트 규칙 준용)
3. 최근 7일 정답률 < 70% → 고빈출 (frequency 8+) 표현 위주로 하향 (spec §5-0 단계 4 준용)
4. 적용한 음소를 payload **`_context: { weakPhonemes: ["/ɹ/", …] }`** 에 기록 (+ `_note` 에 근거 한 줄) — `validate-seed.mjs` 가 표현 phonemes 와 교차 0이면 **경고** (콘텐츠 풀 제약상 차단 아님)
5. 컨텍스트 조회 불가(워크플로 실패 등) 시: 가중 생략 가능 — `_context` 미기록으로 표시
...
- [ ] 🅖 표현 카드 **1~2장** (PPP 집중 추출 — 최소 1장 = 게이트 차단 / 3장 초과 = 과다추출 경고. 구 5~7장 폐기 2026-06-29) — 각 `drills` 4~10개 (en/ko/kr 의무. 개수·품질은 schema §drills 정본 2026-07-22 — 정원 채우기 금지·살아있는 문장·축 다양성·인용형 금지. 전 카드 ≤4 면 게이트 경고)
...
- [ ] 항상 생성 (hold 게이트 폐지 2026-07-01) — 미완료 카운트로 보류하지 않음. seed-supabase 가 INSERT 전 14일+ 방치 미완료를 코드로 강제 정리(`staleIncompleteIds`) → 데드락 재발 불가 (spec §5-0 단계 4). **2026-09-03: 이 자동 정리는 폐지** — 일괄 적재한 코어100 묶음을 지우던 사고 때문이며, 정리는 `expire-stale-lessons.mjs` 수동 실행만. 재적재 게이트는 id 기준이라 완료된 카드는 날짜와 무관하게 차단
...
### phonetic_kr 의 정의 (2026-09-08 사용자 확정 — 이 절이 §7 의 상위 규정이다)

**`phonetic_kr` 은 영어 철자를 한글로 옮긴 독음이 아니다. 그 카드에서 실제로 재생되는 미국 영어
음성이 한국인 귀에 어떻게 들리는지를 최대한 가깝게 옮긴 청각 근사치다.** 아래 규칙은 전부 이
정의에서 따라 나온다. 개별 표기(`아일`/`아을`, `왓 아이`/`와라이`)를 고치는 것은 이 정의를 적용한
결과이지 출발점이 아니다.

1. **판정 기준(SSOT)은 그 카드의 재생 음원이다.** 영어 철자도, 사전 IPA 도, 한국에서 굳은 관습
   표기도 최종 기준이 아니다. 카드가 speaker 를 지정하면 그 voice·rate 가 기준이다(미지정은
   `VOICE_DEFAULTS` + rate 0.85). 재생 속도 0.85 가 연음을 지우지는 않는다 — 실측(2026-09-08):
   같은 문장에서 flap /t/ 길이가 0.85 에서 30~70ms, 1.0 에서 30~50ms 로 둘 다 flap 범위였고,
   `Did you call me?` 는 두 속도 모두 /t/ 가 아예 잡히지 않았다(did you 가 병합돼 발음된다).
2. **자동 점수는 후보를 거르는 도구이지 판정자가 아니다.** Azure 점수가 높다는 것이 "한국인이
   들었을 때 원어민 발음에 가장 가깝다"를 뜻하지 않는다. 근거(2026-09-08 본문 카드 40장 표본):
   이미 연음이 반영된 표기가 낮게 나온다(`어바우릿` 37·42 · `니루` 36 · `나러나` 40 · `가라` 42),
   연음과 무관한 자음군도 같이 낮게 나온다(`쎈스` 37 · `이그잭틀리` 38 · `그뤠잇` 39 · `퀘스천` 40).
   즉 절대 점수로는 '고칠 수 있는 표기 문제'와 '한글로는 원래 못 담는 소리'를 가르지 못한다.
   **쓸 수 있는 형태는 하나뿐이다 — 같은 음원·같은 자리에 후보만 바꿔 대는 상대 비교.**
3. **현상을 미리 정해 전역 치환하지 않는다.** `dark L = 을`, `t = ㄹ` 같은 일괄 규칙은 금지한다.
   같은 표기가 카드에 따라 갈린다 — `와라이` 가 한 카드에서 88, 다른 카드에서 45 였다(2026-09-08).
   주변 모음·강세·위치·화자가 다르면 청감도 다르다.
4. **단어 경계를 보존할 의무가 없다.** 실제 음성에서 경계가 무너지면 한글에서도 무너뜨린다.
   눈으로 익히는 소리 지도와 귀로 들어오는 소리 지도가 어긋나는 것이 이 앱이 막으려는 문제다.
5. **'정확한 영어 발음'과 '한국인에게 어떻게 들리는가'를 혼동하지 않는다.** `아을` 은 /aɪɫ/ 의
   음소 표기가 아니다. 한국인이 그 소리를 재현하는 데 `아일` 보다 나은 청각 단서라는 뜻이다.
6. **앱 채점과 충돌하는 구간은 예외로 다룬다.** 음소를 잃지 않는 연음(flap·linking·dark L)은
   채점에서 손해가 없다 — 오히려 이득이다(2026-09-08 실측: `I'll be there` 를 한국어 화자 5명이
   읽었을 때 화면 점수가 `아일` 50.4 → `아을` 74.4, `I'll see you tonight` 은 66.2 → 86.4).
   반면 음소가 사라지는 강한 어휘 축약은 발음평가가 깎는다(2026-08-31 실측: whaddaya 93.0 → 86.7).
   그런 자리는 음차에 넣기 전에 항목별로 재고, 넣는다면 두 층 표기를 검토한다.

> **결정 대기 (2026-09-08)**
> - **두 층 표기**: 실제 청감(`와러유 두잉 터데이`)과 또박또박형(`왓 아 유 두잉 투데이`)을 함께
>   보여줄지. 카드 스키마·렌더·validate-seed 가 함께 바뀌므로 별도 결정이 필요하다. 부분적으로는
>   `phonemes`/`tips` 필드가 이미 그 역할을 한다(예: "put it → flap, '풋 잇'(X) → t 가 굴러 '푸릿'").
> - **전면 재감사 착수 여부**: 음차가 달린 문장은 1,704건(본문 293 · 드릴 1,411)이다. 자동 점수를
>   절대 기준으로 쓸 수 없으므로(위 2번), 감사는 '현상별 후보 생성 → 카드별 상대 비교 → 차이가 큰
>   카드만 사람이 판정' 의 3단으로만 가능하다. 사람이 볼 분량을 정하는 것이 착수 조건이다.

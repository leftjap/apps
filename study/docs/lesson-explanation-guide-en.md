...
- personal 트랙 저작 규칙(2026-09-19~20 확정, 상세는 `2026-09-13-dialogue-session-work-order.md §7`): 상황은 누구나 겪는 기본 상황만, 장면 하나 = 일기 한 편, 줄마다 일기 출처(`src`) 를 달고 뱅크(`~/apps/tmp/2026-09-19-diary-facts.md`)에 없는 사실은 쓰지 않는다. 첫 줄이 상황을 말해 `situation` 없이도 대화만으로 맥락이 서야 하고, 한 가닥 대화를 먼저 쓴 뒤 줄마다 바로 앞 줄에 답하는지 확인한다. 지오 줄(카드 문장)은 복습 때 혼자 떠올려 다시 쓸 문장만 고르고, 공급원 순위는 ① 30패턴이 든 모두영어 문장 그대로 → ② 모두영어 그대로 → ③ 30패턴 자작 → ④ 233 뱅크. 같은 패턴은 한 장면 안에서 두 카드에 쓰지 않는다. 검증은 `validate-seed`(형식) + `~/apps/tmp/2026-09-19-verify-dialogues.mjs`(출처 게이트) + `2026-09-19-check-sources.mjs`(공급원 대조).

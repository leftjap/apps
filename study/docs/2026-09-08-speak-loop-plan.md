...
1. **미니대화**(작업지시서 §1~§4): `explanation.miniDialogue = [{speaker, en, ko}]` optional 필드, 신규 세션 렌더러(`sessionExprV2.js`) 카드 상단 블록(전체 재생·줄 재생·타깃 줄 강조, 두 화자 음성 교대), 게이트(턴 ≤4, 타깃 줄 = 카드 `sentence` 와 동일·정확히 1회, 주변 줄 ≤10단어), 스키마·가이드 §6.3 문서, 미완료 82장 시드 저작 후 재적재(완료 18장은 id 게이트로 차단됨이 정상). 공유 해설 패널에는 넣지 않는다(복습에서 정답 유출).

...
|---|---|---|---|---|
| 1 — 가나 친숙 | 반응·인사·핵심 종조사 | JLPT 미만 | **0개 (의도적 제외)** | 3~4문장 (한자 0개 특수성 — 초보 부담 가드, 예외 보존) |
| 2 — N5 기반 | 기본 문형·조사·일상 명사·기본 동사 | N5 (어휘 ~800 수준) | N5 한자 도입 (분석 추정 ~100자) | 4~7문장 |
| 3 — N4 진입 | te형·조건문·수수동사·가능형 | N4 | N4 한자 (분석 추정 ~300자) | 6~10문장 |
| 4 — N3 진입 | 자연 속도 회화·애니 따라가기 시작 | N3 (어휘 ~3,750 수준) | N3 한자 (분석 추정 ~650자) | 8~14문장 |
| 5 — N3+ → 목표 도달 | 슬라이스 애니 자막 없이 60~70% / 여행 자유 | N3+ ~ N2 진입 | N3+ | 8~14문장 |
...
가나 친숙도 우선. 종조사 (ね·よ·か·だ·です·な) + 기본 반응 (そう·やばい·まじ·すごい·ありがとう·ごめん·はい·いいえ·うん·だめ) 중심.
- 예: `そうだね` `すごい！` `やばい` `まじで？` `ありがとう` `ごめん` `だめですよ` `またですか？`
...
- **첫 콩트 한자 도입 금지**: 학습 완료 풀이 가나 단편 **10종 미만** 일 때, 콩트 안에 한자 든 stretch 카드 비활성. 모든 카드 Stage 1, 한자 0개. ja 의 첫 한자 도입은 콩트 외 별도 카드 (단순 어휘 카드) 가 안전.
- **stretch 강제 X**: Stage 1~2 단계에서 §5.2 의 stretch 비율 (20~40%) 강제 적용 안 함. 자연 발생 (캐릭터 톤이 자연히 stretch 어휘 — 예: 박사 캐릭터의 학술 어휘) 만 허용. 비율 강제는 **Stage 3+ 부터**.
...
### default + stretch 비율 (Stage 3+ 부터 강제, Stage 1~2 예외)

| 구성 | 비율 | 규칙 |
|---|---|---|
| default 문장 | 60~80% | `stage === currentStage`, `is_stretch: false` |
| stretch 문장 | 20~40% (**Stage 3+ 만**) | `stage === currentStage + 1`, `is_stretch: true` |
| Stage +2 이상 | 금지 | §4 점프 가드 준수 |

### Stage 별 stretch 예외 규칙 (ja 특수)

| Stage | stretch 규칙 |
|---|---|
| 1 (가나 친숙, 학습 풀 < 10종) | **stretch 비활성**. 모든 카드 Stage 1, 한자 0개 |
| 1 (학습 풀 ≥ 10종) | 자연 발생만 허용 (캐릭터 톤). 비율 강제 X |
| 2 (N5 기반) | 자연 발생만 허용 (캐릭터 톤). 비율 강제 X. 한자 도입은 별 카드로 |
| 3+ (N4+) | 비율 강제 (20~40%) |

→ ja 의 한자 도입 부담 + 정중/보통 구분 학습 부담 → Stage 1~2 의 stretch 강제는 학습 인지 부담 폭증. en 의 비율 일괄 적용과 다른 점.

**배치 권장** (Stage 3+ 콩트만): 1번 문장 default (진입 부담 ↓) → stretch 는 중간이나 펀치라인.
...
- [ ] `scene_title` 박힘
- [ ] stretch 문장 비율 — **Stage 3+ 콩트만 20~40% 강제**. Stage 1~2 콩트는 stretch 비활성 또는 자연 발생만 (§4 / §5.2 예외 규칙)
- [ ] Stage 1 콩트 + 학습 풀 < 10종 인 경우 stretch 카드 0건 (모든 카드 Stage 1, 한자 0개)

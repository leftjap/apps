...
1. 문장마다 3회 말하게 하고, 그중 한 번은 사람·시간·사물을 바꾸도록 교사가 질문으로 강제한다(학습자가 스스로 바꾸기를 기다리지 않는다).
2. 문장이나 패턴 이름을 말하지 않고 질문으로만 유도하며, 이미 쓴 질문을 다시 쓰지 않는다.
3. 세션당 3회 이상 "이번엔 저한테 물어보세요" 로 넘기고 학습자 질문에 한 줄로 답한다.
...
**프롬프트 검증**(2026-09-19, 실제 ChatGPT 텍스트 2회): 1차에서 힌트·모델·역할 이탈 교정·be 동사와 주어 교정·관사 통과·칭찬 0회·한 턴 한 질문·문장당 3회·리포트 정확도가 전부 지켜졌고, 변형을 교사가 유도하지 않는 것·되묻기 1회뿐·같은 질문 반복 세 건이 나와 규칙 1~3 을 보강했다. 2차에서 반복 질문 0건, 되묻기 3회, 학습자가 원문 그대로 답하면 무엇을 바꿀지 한국어 세 단어 이내로 지시해 변형을 강제하는 것까지 확인했다. 남은 결함은 3회 세기가 부정확해 한 문장이 2회에서 끝날 수 있다는 것이고, 규칙 줄을 더 늘리면 지시 준수가 떨어지므로(2026-09-15 기록) 그대로 둔다. 화자 이름이 한 번 붙은 사례가 있다. 음성 모드·5초 대기·실제 소요 시간은 미검증. 초안과 추출 스크립트는 `~/apps/tmp/voice-teacher/2026-09-19-gpt-prompt-v2-short.md` 와 같은 폴더의 `extract-base.py`. 빌더를 이 구조로 옮긴 뒤(2026-09-19) 실제 시드 `seeds/en-personal-2026-09-17.json` 로 만든 프롬프트를 ChatGPT 에서 한 번 더 돌려 같은 결과를 확인했다(21턴, 반복 질문 0건, 되묻기 3회, 칭찬 0회, 열두 단어 초과는 리포트뿐, 문장별 판정 정확). 이 실행에서 변형 강제를 한 번 놓쳤다. 상황 배경이 3인칭 한국어 서술인데도 역할을 혼동하지는 않았다. 미니대화는 이 프롬프트에서 쓰지 않는다(2026-09-14 실기에서 대본 대화 구간이 통째로 따라 말하기로 무너졌다).

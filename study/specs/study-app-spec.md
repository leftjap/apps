...
**프롬프트**(`services/voicePrompt.js`, 문자열·객체 입력 공용): 2026-09-19 재설계 2판. ChatGPT 한 경로(클로드 토글 없음).

음성 대화는 학습 세션에서 배운 것을 **숙달**하는 자리다(사용자 정리 2026-09-19). 문항 유형이 하나뿐이면 단조로워 숙달이 안 되므로 패턴마다 여섯 단계를 올라간다.

재료는 패턴마다 세 가지다. 짧은 형태(구문의 `~` 앞부분을 단어 경계까지 보고 포함하는 `drills` 중 최단, 없으면 최단 드릴, 드릴도 없으면 목표 문장), 목표 문장(`sentence`), 나머지 드릴 전부. 슬롯 바꾸기와 응용 질문에 쓸 재료가 필요해서다. 구문(`explanation.key` 좌변)은 짧은 형태에도 목표 문장에도 없을 때만 괄호로 적는다(시드 88%가 문장형 키라 대개 목표 문장과 같다). 질문 소재로 `explanation.situation` 을 중복 없이 이어 붙여 배경 한 줄로 넣는다(앱은 모델을 부르지 않으므로 요약하지 않는다). 미니대화는 넣지 않는다 — 2026-09-14 실기에서 대본 대화 구간이 통째로 따라 말하기로 무너졌다.

단계는 A부터 F까지다.

- A. 짧은 형태를 듣고 따라 말하기
- B. 영어를 듣고 한국어 뜻 말하기 (그 문장을 처음 만났을 때만. 이해 확인이라 주력이 아니다)
- C. 한국어 뜻을 듣고 영어로 말하기
- D. 교사가 한 단어만 바꿔 말하면 따라 말하기. 무엇에 대한 말인지 → 주어 → 시간 순으로 세 번, 한 턴에 하나만
- E. 패턴을 바꾼 질문에 바뀐 답으로 답하기
- F. 답을 듣고 그 답이 나올 질문 만들기

끝내는 조건은 횟수가 아니라 통과다. 힌트 없이 연속 두 번 맞으면 그 단계를 통과하고 한 단계 올라간다. D·E·F 를 통과하면 그 패턴은 오늘 끝이고 "다음으로 갑니다" 뒤 다음 패턴의 A 로 간다. 같은 단계에서 세 번 막히면 접고 다음 패턴으로 간다(끝나지 않는 세션 방지). 마지막 패턴 뒤에는 통과한 패턴으로 대본 없는 짧은 대화를 한다. 리포트는 패턴별 짧은 형태와 통과·힌트·못함 한 단어, 그리고 내일 할 것 한 문장이다.

나머지 규칙은 1판에서 그대로 가져왔다. 한 턴에 하나·5초 대기·열두 단어, 학습자 문장을 대신 말하지 않고 첫 두 단어나 한국어 세 단어로 힌트, 힌트가 안 되면 한 번 들려주고 "따라 하세요" 뒤 두세 턴 뒤 재출제, 상대 대사를 따라 하면 "그건 제 대사예요", be 동사·주어·시제만 교정하고 관사·전치사는 통과, 칭찬 없이 "네", 단계 문자·패턴 번호·괄호는 소리 내지 않기.

**프롬프트 검증**: 1판은 실제 ChatGPT 텍스트 5회로 확인했다. 규칙 줄을 늘리지 말고 기존 줄에 조건과 시점을 박아야 지켜진다는 것이 그때 얻은 결론이다(횟수로 적은 규칙은 세션이 짧은 날 안 지켜졌다). 2판은 27턴 실행에서 첫 패턴이 A부터 F까지 올라가고 "다음으로 갑니다" 뒤 다음 패턴의 A 로 넘어가는 것, 단계 문자와 패턴 번호를 한 번도 말하지 않는 것, 열두 단어 초과와 칭찬이 0인 것을 확인했다. D 의 첫 변경이 명사 교체가 아닌 사례가 나와 문구를 "무엇에 대한 말인지"로 고쳤고, 다시 돌려 그 자리에서 sleeping → eating → 주어 순으로 바뀌는 것까지 확인했다. **미검증**: 마지막 대화 단계와 리포트(패턴 네 개를 다 돌아야 도달한다), 음성 모드, 5초 대기, 실제 소요 시간.

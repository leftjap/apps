...
재료는 목표 문장이 아니라 **그날 세션의 기본 문장 전체**다. 기본 문장은 구문마다 가장 짧은 문장이며, 뽑는 규칙은 앱 세션 ② 단계와 같다(카드 head 를 포함하는 `drills` 중 단어 수가 가장 적은 것). 카드 4장이면 문장 4개가 그대로 한 세션이 된다. 사람·반려동물 등 질문에 필요한 배경은 `explanation.situation` 에서 한 줄로 요약해 넣는다.

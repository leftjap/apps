...
**26회전(2026-09-25, 실제 학습 세션)**. 기본 문장을 목표 문장으로 고정하고, 주어를 바꾸면 틀이 깨지는 문장(`It looks like ~`, `Don't forget to ~` 같은 명령문)은 주어 패스에서 내용·동작을 바꾸게 했다. 실사용 기록에서는 `It looks like rain.` 에 "주어를 지금 날씨로" 가 나와, 학습자가 단서대로 바꾼 `The weather looks like rain` 을 교사가 "it." 으로 되돌리며 힌트를 여섯 번 줬다 — 단서 자체가 틀을 깨라는 요구라 교사도 정답을 정하지 못했다. 완주 실측: 복습·리포트의 네 문장이 모두 배운 문장, 해당 패턴의 주어 패스는 "내용을 만석으로" 로 바뀜, 매번 다르게 틀려도 힌트 두 번 뒤 문장을 들려줌, 단서 낱말 12/12 가 상황·연습 문장 안, 분량 8/12/24 정확, 되묻기 4회 위치 정확, 리포트 판정 일치. **새로 나온 문제**: 대화 시작 턴에 첫 질문이 빠졌고, 8턴 뒤 리포트가 자동으로 오지 않아 통제어로 받았다(23~25회전은 둘 다 정상). 한 턴에 두 가지를 해야 하는 경계 턴에서만 생겼고, 이번 대화의 추론 설정이 Medium(이전 High)이라 원인을 가리지 못했다.

...
세어야 할 것은 하나다. 그 패턴에서 학습자가 말한 문장이 스무 개쯤 되면 "다음으로 갑니다" 를 말하고 다음 패턴으로 간다. 마지막 여섯 개가 도움 없이 연속으로 맞으면 일찍 넘어갈 수 있다. 마지막 패턴 뒤에는 "이제 대화합니다" 를 말하고 대본 없는 대화를 하며, 통제어 `리포트` 로 언제든 끝낼 수 있다. 세션 분량은 "약 15분, 학습자 발화 60회 이상" 으로 준다.
...
**프롬프트 검증**(2026-09-19, 실제 ChatGPT 텍스트 루프 9회전): 회전마다 진단·구현·단위테스트·실행·기록 판정을 돌렸다. 1~6회전은 단계 구조(A~F + 통과 판정)로, 교사가 학습자 문장을 먼저 말하는 앵무새를 없애고(교사 영어 평서문 0, 되돌려준 턴 0), 탈출 조건을 셀 수 있게 바꾸고, 단계·대화·리포트 전환을 한국어로 소리 내어 선언하게 해 지켜지도록 했다. 7회전에서 단계별 최소 문항 수와 슬롯 목록을 얹자 교사가 위치를 잃고 A 방식과 B 방식을 섞었다 — **규칙을 더하지 말고 구조를 단순화해야 한다**는 결론. 8회전에서 회전 구조로 바꾸자 51턴에 학습자 발화 25개가 전부 서로 다른 문장으로 나왔다(이전 판은 같은 문장 되풀이로 1분에 끝났다). 9회전에서 치환을 짧은 형태에 고정해 누적을 없앴다. **미검증**: 음성 모드, 5초 대기, 실제 소요 시간, 패턴 네 개 완주.

...
**27회전(2026-09-25)**. 대화 시작 문구를 "이제 대화합니다 를 말하고 같은 턴에 첫 질문" 으로 고쳤고, 대화 시작 턴에 첫 질문이 나왔다. 해석에 주의할 점: 대화 기록의 model_slug 를 읽어 보니 23~26회전은 전부 gpt-5-6-thinking(23~25 High, 26 Medium)이었는데, 27회전은 질문과 답 후반부터 gpt-5-6(추론 없음)으로 바뀌었다. 같은 시각 화면에는 요금제가 Free 로 뜨고 모델 선택 칸이 사라졌다(세션 정보는 plus 로 남아 있어 추론 모델 사용량 한도로 추정). 그래서 첫 질문 확인은 gpt-5-6 에서 나온 결과다. 리포트는 모델마다 달랐다 — High(23~25)는 마지막 대꾸와 리포트를 둘 다, Medium(26)은 대꾸만, gpt-5-6(27)은 리포트만 했고 문장 대신 틀 이름을 적고 "내일은" 줄을 비웠다. 되묻기는 gpt-5-6 에서 다섯 번(학습자가 질문한 턴 뒤에도 되묻기)으로 늘었다. 틀이 깨지는 주어는 27회전에서 처음 한 번 틀리게 냈다가 학습자가 곧이곧대로 답하자 "제가 잘못 냈어요" 로 바로잡았다. 26·27회전 모두 주어 차례 단서가 규칙 설명의 예시 주어 하나로만 나왔고, 예시 동작 낱말도 한 번 그대로 쓰였다 — 예시 낱말이 단서로 새어 나온다. **남은 과제**: High 에서의 리포트 재확인(한도 때문에 이번엔 못 함), 예시 낱말 누출, 음성 모드.

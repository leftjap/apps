...
- **미니대화 (2026-09-08)**: `explanation.miniDialogue` 가 있으면 정답 공개 뒤 카드 아래에 듣기·녹음 블록(2026-09-12)(§8-3 과 동일 컴포넌트 `miniDialogueEl`). 공개 전에는 DOM 에도 두지 않는다(정답 유출 방지). 근거: 과한 회상도 정답 피드백이 있으면 학습이 됨(Kornell/Klein/Rawson 2015). **2026-09-12**: 줄마다 녹음 버튼 추가(사용자 결정 — '듣기 버튼만' 을 뒤집음). 줄 구성은 응용 행과 동일(영문 · `kr` 발음 · `ko` 뜻 · 듣기 · 녹음). 채점·점수 원·오늘 발화·`#mini#` 이력은 응용 행과 같고 진행 조건·SRS 영향 없음. 턴 한도 2~8, 음성은 화자 성별(A/B)로만 구분, `name` 표시, `situation` 을 장면 한 줄로 표시.
...
- **듣기 음성**: 대화 줄과 선택 줄 듣기 필은 화자 성별 고정(`MINI_VOICES` A 여성 · B 남성)에 `rate 1.0`. 대화가 없는 단독 줄만 기존 문장 카드 규칙(`PRACTICE_VOICES` 순환 · 기본 속도 0.85)을 쓴다. 응용 행은 기존 `pickPracticeVoice`(길이별 속도) 그대로. 전체 듣기는 줄마다 `onEnd` 체인이고 선택 줄 차례에는 듣기 필이 재생 표시를 받는다.
- **응용 행**: `drillRows({ threeLine: true })` — 영문 / `[발음]` / 뜻 세 줄 + 문장 아래 흔적 줄. 점수 원을 버튼 옆 가로에 두지 않는다(원 6개면 문장이 눌린다). 복습은 옵션 없이 부르므로 종전 한 줄 부제를 유지한다.
- **미니대화 블록 (2026-09-08 신설 · 2026-09-12 녹음 추가 · 2026-09-14 부터 복습 전용)**: `miniDialogueEl` 은 신규 세션에서 대화 스테이지로 대체됐고 복습 세션만 쓴다. `explanation.miniDialogue`(2~8턴) 가 있으면 문장 카드 바로 아래에 "이런 대화에서" 블록 — 전체 듣기·한 줄 듣기·타깃 줄 강조·한글 병기. 판정·잠금 없음(녹음은 2026-09-12 추가). 행 구조는 응용 행(.vs-drow)과 같아 버튼 열이 정렬된다. 코어100 100장 전부 적용. **복습 세션(§8-2)에도 같은 블록** — 단 타깃 줄이 정답이라 정답 공개 뒤에만 생성해 카드 아래 슬롯에 붙인다. 정본 `docs/explanation-schema.md §miniDialogue` **2026-09-12**: 줄마다 녹음 버튼 추가(사용자 결정 — '듣기 버튼만' 을 뒤집음). 줄 구성은 응용 행과 동일(영문 · `kr` 발음 · `ko` 뜻 · 듣기 · 녹음). 채점·점수 원·오늘 발화·`#mini#` 이력은 응용 행과 같고 진행 조건·SRS 영향 없음. 턴 한도 2~8, 음성은 화자 성별(A/B)로만 구분, `name` 표시, `situation` 을 장면 한 줄로 표시.
- 카운터: "신규 N / total"
- 오디오 재생 버튼 (Azure TTS)
- "따라 말하기" 발음 연습 버튼
- 해설 패널 (접기/펼치기):

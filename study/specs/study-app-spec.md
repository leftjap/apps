...
**프롬프트**(`services/voicePrompt.js`, 문자열·객체 입력 공용): 2026-09-19 재설계. ChatGPT 한 경로(클로드 토글 없음).

재료는 목표 문장이 아니라 **그날 세션의 기본 문장 전체**다. 기본 문장은 구문마다 가장 짧은 문장이다. 구문(`explanation.key` 좌변)의 `~` 앞부분을 포함하는 `drills` 중 단어 수가 가장 적은 것을 고르고, 없으면 가장 짧은 드릴, 드릴도 없으면 목표 문장을 쓴다. 세션 시안의 ② 단계와 같은 뜻이지만 그 시안은 아직 앱에 없다. 맞춤은 단어 경계까지 본다. 부분 문자열로 찾으면 `I keep` 이 `Nani keeps` 안에 걸려 학습자가 배운 형태가 아닌 문장이 뽑힌다(`en-personal-2026-09-17` 카드 2에서 실제로 발생). 카드 4장이면 문장 4개가 그대로 한 세션이 된다. 질문 소재가 되도록 카드의 `explanation.situation` 을 중복 없이 이어 붙여 배경 한 줄로 넣는다(앱은 모델을 부르지 않으므로 요약하지 않고 원문 그대로 넣는다).

단계 대본을 두지 않는다. 대본과 대사 목록을 주면 교사가 목록 밖 표현을 지어내고 학습자는 빈칸만 채우게 된다(2026-09-10 v4 실측 `~/apps/tmp/voice-teacher/runs-analysis-v4.md`). 목적·재료·규칙만 스무 줄 안에 준다. 규칙은 여덟 줄이다.

1. 문장마다 3회 말하게 하되 맞게 말한 것만 센다(엉뚱하거나 틀린 답은 세지 않는다). 그중 한 번은 사람·시간·사물을 바꾸도록 교사가 질문으로 강제하고, 학습자가 바꾸지 않고 답하면 무엇을 바꿀지 한국어 세 단어 이내로 말하고 기다린다.
2. 문장이나 패턴 이름을 말하지 않고 질문으로만 유도하며, 이미 쓴 질문을 다시 쓰지 않는다.
3. 학습자의 네 번째 답마다 "이번엔 저한테 물어보세요" 로 넘기고 학습자 질문에 한 줄로 답한다(횟수로 적으면 세션이 짧은 날 못 지킨다).
4. 한 턴에 질문 하나, 5초 대기, 교사 발화는 한 줄 열두 단어 이내.
5. 학습자 문장을 대신 말하지 않는다. 힌트는 첫 두 단어 또는 한국어 세 단어 이내.
6. 힌트로 안 되면 문장을 한 번 들려주고 "따라 하세요", 두세 턴 뒤 같은 문장을 다시 묻는다.
7. 학습자가 상대 대사를 따라 하면 "그건 제 대사예요" + 첫 두 단어.
8. be 동사·주어·시제는 반드시 고치고 관사·전치사는 넘긴다. 칭찬 없이 "네" 하고 바로 다음.

네 문장이 3회씩 나오면 멈추고 문장별 혼자/힌트/모델 한 단어와 내일 연습할 것 한 문장을 낸다. 옛 규칙(맥락 전이, 타깃 숨기기, 퀴즈 금지, 2026-09-15 단계 대본)은 이것으로 대체된다.

**프롬프트 검증**(2026-09-19, 실제 ChatGPT 텍스트 5회 — 손으로 쓴 초안 2회 + 앱이 만든 프롬프트 3회): 1차에서 힌트·모델·역할 이탈 교정·be 동사와 주어 교정·관사 통과·칭찬 0회·한 턴 한 질문·문장당 3회·리포트 정확도가 지켜졌고, 변형을 교사가 유도하지 않는 것·되묻기 1회뿐·같은 질문 반복이 나와 규칙 1~3 을 보강했다. 2차에서 반복 질문 0건, 되묻기 3회, 학습자가 원문 그대로 답하면 한국어 세 단어 이내로 변형을 강제하는 것을 확인했다. 3차(앱이 만든 프롬프트, 21턴)에서 변형 강제를 두 번 중 한 번 놓치고 엉뚱한 답을 3회에 세어 한 문장이 2회에서 끝났다. 그래서 규칙 줄을 늘리지 않고 1번 줄에 세는 조건과 안 바꿨을 때의 대응을 넣었다. 4차에서 그 두 가지가 고쳐졌으나(변형 강제 3회 모두 성공, 네 문장 모두 정확히 3회) 세션이 16턴으로 짧아져 되묻기가 2회에 그쳤다. 되묻기를 횟수 대신 "네 번째 답마다" 로 바꾼 5차에서 22턴·질문 12개·반복 0건·되묻기 4회(답 4·9·13·17번째)·칭찬 0회, 열두 단어 초과는 리포트뿐, 문장별 판정도 실제 기록과 일치했다. 남은 미검증은 음성 모드·5초 대기·실제 소요 시간이며 텍스트로는 확인할 수 없다. 초안과 추출 스크립트는 `~/apps/tmp/voice-teacher/2026-09-19-gpt-prompt-v2-short.md` 와 같은 폴더의 `extract-base.py`. 미니대화는 이 프롬프트에서 쓰지 않는다(2026-09-14 실기에서 대본 대화 구간이 통째로 따라 말하기로 무너졌다).

...
- **내 이야기 트랙 (en `track: mylife`, 2026-09-10~)**: 즉석 생산이 A1 인 학습자용 5일치 — 기본동사 머리·3~7단어·내 상황(아내·고양이·일상·여행) 문장, 한글 프롬프트는 회상 수렴형, 모두영어 8~13편 문장 재활용. 코어100 과 같은 표현 전용 세션 형식이고 같은 로더로 열린다. date 는 남은 코어100 묶음보다 앞(2026-08-27~)이라 먼저 열린다. 정본 `docs/mylife-curriculum.md`.
...
- **미니대화 (2026-09-08)**: `explanation.miniDialogue` 가 있으면 정답 공개 뒤 카드 아래에 듣기 전용 블록(§8-3 과 동일 컴포넌트 `miniDialogueEl`). 공개 전에는 DOM 에도 두지 않는다(정답 유출 방지). 근거: 과한 회상도 정답 피드백이 있으면 학습이 됨(Kornell/Klein/Rawson 2015).
...
### 8-3. 신규 레슨 카드
- 카드 표시: 문장, 읽기(일본어), 의미
- **미니대화 블록 (2026-09-08, 카드 아래, 듣기 전용)**: `explanation.miniDialogue`(2~4턴) 가 있으면 문장 카드 바로 아래에 "이런 대화에서" 블록 — 전체 듣기·한 줄 듣기·타깃 줄 강조·한글 병기. 녹음·판정·잠금 없음(사용자 최종 결정 2026-09-08 "듣기 버튼만"). 행 구조는 응용 행(.vs-drow)과 같아 버튼 열이 정렬된다. 코어100 100장 전부 적용. **복습 세션(§8-2)에도 같은 블록** — 단 타깃 줄이 정답이라 정답 공개 뒤에만 생성해 카드 아래 슬롯에 붙인다. 정본 `docs/explanation-schema.md §miniDialogue`
- 카운터: "신규 N / total"
- 오디오 재생 버튼 (Azure TTS)
- "따라 말하기" 발음 연습 버튼
- 해설 패널 (접기/펼치기):

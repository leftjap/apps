...
### 8-3. 신규 레슨 카드
- 카드 표시: 문장, 읽기(일본어), 의미
- **미니대화 블록 (2026-09-08, 카드 아래)**: `explanation.miniDialogue`(2~4턴) 가 있으면 문장 카드 바로 아래에 "이런 대화에서" 블록 — 전체 듣기·한 줄 듣기·타깃 줄 강조·한글 병기, 줄마다 선택 녹음(응용 행과 같은 채점·점수 배지·발화 집계, `#mini#` 로그). 진행 조건(판정·잠금·통과 기준)은 없음. 복습 화면에는 없음. 정본 `docs/explanation-schema.md §miniDialogue`
- 카운터: "신규 N / total"
- 오디오 재생 버튼 (Azure TTS)
- "따라 말하기" 발음 연습 버튼
- 해설 패널 (접기/펼치기):

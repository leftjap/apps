...
### 8-3. 신규 레슨 카드
- 카드 표시: 문장, 읽기(일본어), 의미
- **미니대화 블록 (2026-09-08, 카드 아래, 듣기 전용)**: `explanation.miniDialogue`(2~4턴) 가 있으면 문장 카드 바로 아래에 "이런 대화에서" 블록 — 전체 듣기·한 줄 듣기·타깃 줄 강조·한글 병기. 녹음·판정·잠금 없음(사용자 최종 결정 2026-09-08 "듣기 버튼만"). 행 구조는 응용 행(.vs-drow)과 같아 버튼 열이 정렬된다. 코어100 100장 전부 적용. **복습 세션(§8-2)에도 같은 블록** — 단 타깃 줄이 정답이라 정답 공개 뒤에만 생성해 카드 아래 슬롯에 붙인다. 정본 `docs/explanation-schema.md §miniDialogue` **2026-09-12**: 줄마다 녹음 버튼 추가(사용자 결정 — '듣기 버튼만' 을 뒤집음). 줄 구성은 응용 행과 동일(영문 · `kr` 발음 · `ko` 뜻 · 듣기 · 녹음). 채점·점수 원·오늘 발화·`#mini#` 이력은 응용 행과 같고 진행 조건·SRS 영향 없음. 턴 한도 2~8, 음성은 화자 성별(A/B)로만 구분, `name` 표시, `situation` 을 장면 한 줄로 표시.
- 카운터: "신규 N / total"
- 오디오 재생 버튼 (Azure TTS)
- "따라 말하기" 발음 연습 버튼
- 해설 패널 (접기/펼치기):

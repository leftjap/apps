...
4. **최근 학습 이력 조회** (`study_today_lessons` + `study_session_logs` SELECT)
   ...
   - **항상 생성 (2026-07-01 데드락 근본 수정 — "미완료 5건 초과 보류" 게이트 폐지)**: 미완료 카운트로 신규 생성을 **보류하지 않는다**. 대신 `seed-supabase.mjs` 가 INSERT 전 **14일+ 방치 미완료를 코드로 강제 삭제**(`staleIncompleteIds`). 옛 hold 게이트는 에이전트 판단이라, 완료 불가·방치 미완료(2026-06-08 결함 배치)가 미완료>5 를 고정해 en 을 3주간 매일 HOLD 시킨 데드락의 원인이었음(수학은 이 게이트가 없어 매일 성공). 강제 정리는 코드가 하므로 데드락 재발 불가. **(2026-09-03 갱신: 이 자동 강제 삭제는 폐지.** 일괄 적재한 코어100 묶음은 방치가 아니라 대기인데 이 삭제가 미리 넣은 묶음을 지웠다(2026-08-28 ja 12장). 정리는 `expire-stale-lessons.mjs` 를 사람이 직접 돌릴 때만. 재적재 게이트는 id 기준이라 완료된 카드는 날짜와 무관하게 차단.)
...
- **코어100 (en 2026-08-26~ / ja 2026-08-28~)**: 데몬 없이 **일괄 적재**. 시드 파일 하나 = 세션 하나, `date` 는 달력이 아니라 **세션 순서**(하루씩 증가). 로더(`src/pages/cardLoader.js` loadNewCards)가 오늘까지의 미완료 중 가장 오래된 날짜 묶음 하나만 열고, 오늘까지의 묶음을 다 끝냈으면 다음 날짜 묶음을 당겨 연다(2026-09-03). 홈 신규 카운트 = 그 묶음의 표현 수(같은 로더 출처). 세션 생성 자동화는 신설하지 않는다(사용자 결정 2026-09-03)
...
> **폐기(2026-07-10)** — 코드는 Rung 없이 단일 회상 모드다(`isRecallMode`: en 항상 회상, ja 는 interval ≥ 3). 아래 내용은 이력으로만 남긴다. (2026-09-13 문서 정정)
...
- **피드백 필수**: Rung 2/3 은 첫 녹음 직후 영어 정답 즉시 공개(`reveal`).
- **미니대화 (2026-09-08)**: `explanation.miniDialogue` 가 있으면 정답 공개 뒤 카드 아래에 듣기·녹음 블록(2026-09-12)(§8-3 과 동일 컴포넌트 `miniDialogueEl`). 공개 전에는 DOM 에도 두지 않는다(정답 유출 방지). 근거: 과한 회상도 정답 피드백이 있으면 학습이 됨(Kornell/Klein/Rawson 2015). **2026-09-12**: 줄마다 녹음 버튼 추가(사용자 결정 — '듣기 버튼만' 을 뒤집음). 줄 구성은 응용 행과 동일(영문 · `kr` 발음 · `ko` 뜻 · 듣기 · 녹음). 채점·점수 원·오늘 발화·`#mini#` 이력은 응용 행과 같고 진행 조건·SRS 영향 없음. 턴 한도 2~8, 음성은 화자 성별(A/B)로 나누고 누를 때마다 같은 성별 안에서 바뀜(2026-09-25, 아래 '듣기 음성'), `name` 표시, `situation` 을 장면 한 줄로 표시.
...
### 8-3. 신규 레슨 카드
- 카드 표시: 문장, 읽기(일본어), 의미
- **대화 스테이지 (2026-09-14 시안 12a, 문장 카드 폐기)**: 신규 세션 화면은 문장 카드(`.vs-card`)를 그리지 않는다. 표현 카드를 `explanation.miniDialogue` 기준으로 묶어(`buildDialogueGroups`, `components/session/applied.js`) 대화를 위에 고정하고, 선택한 카드의 줄이 그 자리에서 열려 듣기·따라 말하기 필·점수 링·본 점수 열을 품는다. 카드 줄 판정은 `line.en.trim() === card.sentence.trim()`, 선택은 `state.step`. 대화가 없거나 카드 문장이 그 대화에 없는 카드는 카드 문장 한 줄짜리 **단독 묶음**이 되고(라벨·장면·전체 듣기 없음, 연속하면 헤어라인으로 이어 한 열), 카드마다 대화가 다르면 묶음이 위→아래로 나열된다. 번호는 세션 전체 순번. 데스크톱은 좌측 사이드바 250(진행·세그먼트바·문장 목록·오늘 발화·공부 이력·세션 종료) / 대화 / 우측 400(응용·해설·다음 표현), 폰은 상단 바(세그먼트바 클릭 이동, **스텝 줄·장면 칩 폐기**) / 대화(접기 토글 — 직전 상대 줄 + 선택 줄) / 응용 / 해설 / 위젯 / 하단 CTA.
- **점수 표시**: 선택 줄 본 점수 열은 최근 10개(`MAIN_DOTS_MAX`)이고 이력이 있을 때만 그린다. 접힌 카드 줄·상대 줄·응용 행의 흔적 줄은 최근 8개 + `+N`(`DRILL_DOTS_MAX`). 접힌 카드 줄은 뜻 끝에 `· 응용 d/총`, 말한 카드는 배지가 체크. 좌측 문장 목록에는 `말하기 N회 · 응용 d/총` 과 마지막 점수 원.
- **듣기 음성**: 대화 줄과 선택 줄 듣기 필은 `rate 1.0` 이고, 누를 때마다 같은 성별 안에서 목소리가 바뀐다(2026-09-25 사용자 결정 — 종전엔 성별마다 한 목소리로 고정). A(여성)는 Ava·Emma, B(남성)는 Andrew·Guy·Eric 순서이고 첫 목소리는 종전 고정값(`MINI_VOICES`)이다. 누른 횟수는 줄 문장별로 세션 state(`voiceTurns`)에 남아 카드를 옮겨도 이어지며, 선택 줄 듣기 필과 그 줄의 원 버튼이 같은 횟수를 쓴다. 전체 듣기는 한 번 재생하는 동안 인물마다 한 목소리를 유지하고 다시 누르면 다음 목소리로 넘어간다. 복습의 미니대화 블록과 상대 줄 단서도 같은 규칙이다(`nextMiniVoice`). 대화가 없는 단독 줄만 기존 문장 카드 규칙(`PRACTICE_VOICES` 순환 · 기본 속도 0.85)을 쓴다. 응용 행은 기존 `pickPracticeVoice`(길이별 속도) 그대로. 전체 듣기는 줄마다 `onEnd` 체인이고 선택 줄 차례에는 듣기 필이 재생 표시를 받는다.
- **응용 행**: `drillRows({ threeLine: true })` — 영문 / `[발음]` / 뜻 세 줄 + 문장 아래 흔적 줄. 점수 원을 버튼 옆 가로에 두지 않는다(원 6개면 문장이 눌린다). 복습은 옵션 없이 부르므로 종전 한 줄 부제를 유지한다.
- **미니대화 블록 (2026-09-08 신설 · 2026-09-12 녹음 추가 · 2026-09-14 부터 복습 전용)**: `miniDialogueEl` 은 신규 세션에서 대화 스테이지로 대체됐고 복습 세션만 쓴다. `explanation.miniDialogue`(2~8턴) 가 있으면 문장 카드 바로 아래에 "이런 대화에서" 블록 — 전체 듣기·한 줄 듣기·타깃 줄 강조·한글 병기. 판정·잠금 없음(녹음은 2026-09-12 추가). 행 구조는 응용 행(.vs-drow)과 같아 버튼 열이 정렬된다. 코어100 100장 전부 적용. **복습 세션(§8-2)에도 같은 블록** — 단 타깃 줄이 정답이라 정답 공개 뒤에만 생성해 카드 아래 슬롯에 붙인다. 정본 `docs/explanation-schema.md §miniDialogue` **2026-09-12**: 줄마다 녹음 버튼 추가(사용자 결정 — '듣기 버튼만' 을 뒤집음). 줄 구성은 응용 행과 동일(영문 · `kr` 발음 · `ko` 뜻 · 듣기 · 녹음). 채점·점수 원·오늘 발화·`#mini#` 이력은 응용 행과 같고 진행 조건·SRS 영향 없음. 턴 한도 2~8, 음성은 화자 성별(A/B)로 나누고 누를 때마다 같은 성별 안에서 바뀜(2026-09-25, 아래 '듣기 음성'), `name` 표시, `situation` 을 장면 한 줄로 표시.
- 카운터: "신규 N / total"
- 오디오 재생 버튼 (Azure TTS)
- "따라 말하기" 발음 연습 버튼
- 해설 패널 (접기/펼치기):
...
**화면(`src/pages/listen.js`)**: 상단 "홈으로", 제목 "연속 듣기", 부제 "영어 N문장 · 한 바퀴 약 M분", 큰 재생/일시정지 버튼, 만드는 중 진행률("소리 만드는 중 2/5"), "다시 만들기", 그 아래 **스크립트 목록**(2026-09-06 사용자 제안). 한 줄 = 번호 · 한글 원문(괄호 힌트 포함, 표시용) · 외국어 문장. 재생 위치가 바뀌면 현재 문장 한 줄만 강조(`.cur`)하고 가운데로 자동 스크롤하며, 줄을 누르면 그 문장 시작으로 옮겨 재생한다. 문장별 시작 초는 합성 SSML 의 한글 voice 첫머리에 넣은 `<bookmark mark="쌍 번호"/>` 로 받는다(SDK `bookmarkReached`, `audioOffset` 100ns 틱, 스피커 없이도 발생 — 2026-09-06 실측 두 번째 쌍 5.372초). 표식이 안 오는 묶음(데모 합성기 등)은 묶음 길이를 문장 수로 균등 분할한 근사값을 쓴다. 잠금화면 패널 제목에 현재 문장을 띄우는 것은 실기기에서 백그라운드 JS 동작이 확인되지 않아 보류. 속도/쉼 설정·캐시는 범위 밖.
...
**목적**: 배운 표현을 ChatGPT 음성 모드에서 실제 대화로 꺼내 쓰게 한다(즉석 인출·턴 교대·예측 못 한 후속 질문·맥락 전이). 세션 요약에서만 나오던 프롬프트를 홈에서 언제든 다시 만든다. 앱 안에서 모델을 호출하지는 않는다(추가 비용 0 원칙 유지).
...
음성 대화는 학습 세션에서 배운 것을 **숙달**하는 자리다(사용자 정리 2026-09-19). 문항 유형이 하나뿐이면 단조로워 숙달이 안 되므로 여섯 가지 질문 방식을 돌린다.

재료는 패턴마다 두 가지다. 기본 문장(앱 세션에서 배운 목표 문장 `sentence`, 없으면 구문)과 드릴 전부. 드릴은 치환과 응용 질문의 재료다. 2026-09-19 부터 기본 문장을 "구문을 포함하는 드릴 중 최단" 으로 골랐으나, 그러면 구문을 담은 드릴이 하나라도 있을 때 목표 문장이 쓰이지 않는다. 2026-09-25 실사용에서 네 패턴 모두 배운 문장 대신 응용 문장으로 연습돼 목표 문장으로 고정했다(22~25회전 검증도 같은 이유로 전부 응용 문장으로 돌았다). 구문(`explanation.key` 좌변)은 기본 문장과 다를 때만 괄호로 적는다(시드 88%가 문장형 키라 대개 같다). 질문 소재로 `explanation.situation` 을 중복 없이 이어 붙여 배경 한 줄로 넣는다(앱은 모델을 부르지 않으므로 요약하지 않는다). 미니대화는 넣지 않는다 — 2026-09-14 실기에서 대본 대화 구간이 통째로 따라 말하기로 무너졌다.

진행은 **여섯 가지 단서를 순서대로 돌리는 회전**이다. 단계와 통과 판정을 따로 세게 만들면 교사가 자기 위치를 잃는다(2026-09-19 8회전 실측).

1. 전체 뜻을 한국어로 → 학습자가 영어로 말한다
2. 주어만 바꿀 것을 한국어로("소연으로") → 학습자가 문장 전체를 만든다
3. 대상이나 행동만 바꿀 것을("운동으로")
4. 시간이나 장소만 바꿀 것을("이번 주로")
5. 교사가 영어로 질문 → 학습자가 무언가를 바꿔 영어로 답한다
6. 교사가 "이번엔 질문을 만드세요" 를 말한 뒤 한국어로 답을 주면 → 학습자가 그 답이 나올 영어 질문을 만든다

**모든 치환은 짧은 형태에서 시작한다.** 직전 문장에서 이어 바꾸면 치환이 누적돼 학습자가 문장 전체를 기억해야 하고(9회전 실측), 사용자가 기피한 체이닝이 된다.

세어야 할 것은 하나다. 패턴당 목표 문장 수는 빌더가 패턴 수로 나눠 계산한다(`52 / 패턴 수`, 최소 6). 네 패턴이면 13이다. 고정값 스무 개를 쓰면 "발화 60회" 와 모순돼 네 패턴 세션이 끝나지 않는다(2026-09-20 무수정 완주에서 58턴에 세 번째 패턴 중간이었다). 목표에 닿으면 "다음으로 갑니다" 를 말하고 다음 패턴으로 간다. 마지막 여섯 개가 도움 없이 연속으로 맞으면 일찍 넘어갈 수 있다. 마지막 패턴 뒤에는 "이제 대화합니다" 를 말하고 대본 없는 대화를 하며, 통제어 `리포트` 로 언제든 끝낼 수 있다. 세션 분량은 "약 15분, 학습자 발화 60회 이상" 으로 준다.

**교사가 학습자의 영어 문장을 먼저 말하지 않는다.** 교사의 영어는 질문뿐이고 나머지는 한국어다. 예외는 두 가지로, 패턴 시작에 짧은 형태를 한 번 들려주고 따라 하게 하는 것(채점 안 함)과 힌트가 두 번 실패한 뒤 모델을 한 번 주는 것이다. 이 줄이 없으면 교사가 매 턴 정답을 말하고 학습자는 되돌려주기만 하는 앵무새가 된다(2026-09-19 사용자 지적).

맞았을 때만 "네" 로 시작하게 한다. 짧은 한국어 단서는 그 표시가 없으면 새 문항인지 교정인지 구분되지 않는다.

**프롬프트 검증**(2026-09-19~20, 실제 ChatGPT 텍스트 루프 19회전): 회전마다 진단·구현·단위테스트·완주·기록 판정을 돌렸다. 핵심 교훈 다섯 가지. ① 교사가 학습자의 영어 문장을 먼저 말하면 앵무새가 된다(드릴 구간에서 교사의 영어는 질문만). ② 규칙을 더하면 준수율이 떨어진다 — 단계별 최소 문항·통과 판정·탈출 조건을 동시에 세게 하자 교사가 위치를 잃었고, 회전 구조로 단순화해야 풀렸다. ③ 마커는 말한 뒤 같은 턴에서 무언가를 해야 지켜진다 — 말하고 기다리는 턴은 건너뛴다. ④ 조건은 셀 수 있게 써야 발동한다("같은 단계 세 번" → "한 단계에서 네 번 틀리면"). ⑤ 대화 구간은 드릴 규칙을 끊고 상대에게 역할을 줘야 대화가 된다. **무수정 완주 실측(2026-09-20)**: 발화 93회(드릴 83 + 대화 10), 패턴 전환 3회로 네 패턴을 다 돌고 대화와 리포트까지 도달. 바퀴 선언 8회, 서로 다른 문장 81개, 드릴 모델 제시 4회, 대화 교정·AI 발언 0, 되묻기 4회, 학습자 질문 4개 전부 영어로 답함, 리포트 마커 1회에 괄호 표기 0, 재요청에 "끝났습니다". **남은 과제**: 드릴 교정 비율이 27% 로, 두 패턴 축약 실행의 10% 보다 높다. 회전이 빨라지면 학습자가 기준을 놓친다. **미검증**: 음성 모드, 5초 대기, 실제 소요 시간.

**단계 구조 재설계(20~21회전, `e856825e`·`3f1cbebc`)**: 위 문단의 패턴 중심 회전(패턴당 목표 문장 수·"다음으로 갑니다")은 폐기됐고, 현행은 복습(2n) → 바꿔 말하기(슬롯별 패스, 6n) → 질문과 답(4n) → 대화(학습자 8턴) → 리포트의 단계 중심이다. 정본은 빌더 `src/services/voicePrompt.js` 와 `~/apps/tmp/voice-teacher/2026-09-20-handoff.md` 이며 이 소절의 구조 서술은 아직 개정 전이다. **22~23회전 무수정 완주 실측(2026-09-20, 로그 `tmp/voice-teacher/runs-r22/`)**: 복습 8·바꿔 말하기 24·질문과 답 16 이 정확히 채워지고, 리포트는 별도 턴에 마커로 열리며 혼자/힌트/못함 판정이 실제 힌트·모델 기록과 일치했다. 대화 되묻기는 4회 목표에 3회였다. 원인은 종료(학습자 턴 8)와 되묻기(교사 질문 2·4·6·8번째)가 다른 것을 세는 데 있었다 — 되묻기마다 학습자 질문과 교사 답변에 한 턴이 빠져 학습자 8턴 안에 교사 질문은 7개까지만 나온다. 되묻기 시점을 학습자 턴(1·3·5·7번째 뒤)으로 옮겨 한 카운터로 합쳤다. 두 회전 연속 재현된 미수정 결함: 짧은 형태가 목표 문장보다 긴 카드에서 예고 뜻(goal)과 읽는 문장(short)이 어긋남, 명령문에 주어 패스가 걸림.

**25회전(2026-09-20, `903fd740`)에서 고친 세 가지**. ① 문장 이름을 짧은 형태의 한국어로 못박아 예고 뜻 불일치를 없앴다(22~24회전 내내 재현되던 결함). ② 바꿔 말하기 단서의 낱말이 학습자의 하루와 무관하게 나왔다 — 배경 줄이 `so your questions make sense` 로 질문에만 한정돼 교사가 단서에는 쓰지 않았기 때문이다. 배경 줄을 단서까지 덮게 고치고 낱말 출처를 명시해 12발화 중 위반이 2건으로 줄었다(100% 는 아니다). ③ 분량을 2n/6n/4n 에서 2n/3n/6n 으로 옮겼다 — 기계적인 치환이 24, 의미가 붙는 인출이 16이라 거꾸로였다. **25회전 완주 실측**: 바꿔 말하기 12·질문과 답 24 정확, 되묻기 4회 위치 정확, 리포트 판정이 실제 기록과 일치. **오답 경로를 처음 검증했다** — 동명사·3인칭 -s 누락에 교사가 정답을 말하지 않고 고칠 곳만 짚었고("wake → waking으로요", "keeps로요"), 엉뚱한 답에는 슬롯 단서 대신 "깨는 질문이요" 로 방향만 줬으며, 셋 다 "네" 로 시작하지 않아 새 문항과 구분됐다. **남은 결함**: 명령문에 주어 패스가 걸리는 것, 질문 만들기에서 명령문을 답으로 제시하는 것, 단서 낱말 출처 위반 2건.

**26회전(2026-09-25, 실제 학습 세션)**. 기본 문장을 목표 문장으로 고정하고, 주어를 바꾸면 틀이 깨지는 문장(`It looks like ~`, `Don't forget to ~` 같은 명령문)은 주어 패스에서 내용·동작을 바꾸게 했다. 실사용 기록에서는 `It looks like rain.` 에 "주어를 지금 날씨로" 가 나와, 학습자가 단서대로 바꾼 `The weather looks like rain` 을 교사가 "it." 으로 되돌리며 힌트를 여섯 번 줬다 — 단서 자체가 틀을 깨라는 요구여서 교사도 정답을 정하지 못한 것으로 보인다(추정). 완주 실측: 복습·리포트의 네 문장이 모두 배운 문장, 해당 패턴의 주어 패스는 "내용을 만석으로" 로 바뀜, 매번 다르게 틀려도 힌트 두 번 뒤 문장을 들려줌, 단서 낱말 12개 중 6개가 상황·연습 문장에서 나오고, 3개는 규칙 설명의 예시 단서에 쓰인 주어를 그대로 가져왔으며, 3개는 자료에 없는 시간 낱말(지금·오늘)이었다(연습 문장은 영어만 들어가서 예시 속 이름은 자료에 없다), 분량 8/12/24 정확, 되묻기 4회 위치 정확, 리포트 판정 일치. **새로 나온 문제**: 대화 시작 턴에 첫 질문이 빠졌고, 8턴 뒤 리포트가 자동으로 오지 않아 통제어로 받았다(23~25회전은 둘 다 정상). 한 턴에 두 가지를 해야 하는 경계 턴에서만 생겼고, 이번 대화의 추론 설정이 Medium(이전 High)이라 원인을 가리지 못했다.

**27회전(2026-09-25)**. 대화 시작 문구를 "이제 대화합니다 를 말하고 같은 턴에 첫 질문" 으로 고쳤고, 대화 시작 턴에 첫 질문이 나왔다. 해석에 주의할 점: 대화 기록의 model_slug 를 읽어 보니 23~26회전은 전부 gpt-5-6-thinking(23~25 High, 26 Medium)이었는데, 27회전은 대화 2턴째 답부터 gpt-5-6(이름에 thinking 이 없는 모델)으로 바뀌었다. 몇 분 뒤 확인한 화면에는 요금제가 Free 로 뜨고 모델 선택 칸이 사라져 있었다(로그인 세션 정보는 plus 로 남아 있어 추론 모델 사용량 한도로 추정). 첫 질문은 전환 전 gpt-5-6-thinking(Medium)이 냈다 — 26회전에서 실패한 것과 같은 모델·설정에서 수정이 확인된 것이다. 리포트는 모델마다 달랐다 — High(23~25)는 마지막 대꾸와 리포트를 둘 다, Medium(26)은 대꾸만, gpt-5-6(27)은 리포트만 했고 문장 대신 틀 이름을 적고 "내일은" 줄을 비웠다. 되묻기는 다섯 번으로 늘었는데, 규칙에 없는 두 번(학습자가 질문한 4·8턴 뒤)은 둘 다 gpt-5-6 에서 나왔다. 틀이 깨지는 주어는 27회전에서 처음 한 번 틀리게 냈다가 학습자가 곧이곧대로 답하자 "제가 잘못 냈어요" 로 바로잡았다. 26·27회전 모두 주어 차례 단서가 규칙 설명의 예시 주어 하나로만 나왔고, 예시 동작 낱말도 한 번 그대로 쓰였다 — 예시 낱말이 단서로 새어 나온다. **남은 과제**: High 에서의 리포트 재확인(한도 때문에 이번엔 못 함), 예시 낱말 누출, 음성 모드.

**사용 절차**: 복사 → ChatGPT 새 대화에 붙여 넣어 보내기 → 같은 대화에서 음성 모드 시작 → 약 10분. 효과 지표(작업지시서 검토 결정): 4주 뒤 대화 기록에서 타깃 표현을 힌트 없이 쓴 횟수.

**후속(별도 계획)**: 신규 세션 미니대화(`explanation.miniDialogue`, 작업지시서 §1~§4) → 복습 단서 다양화(2회차부터 `drills[].ko`·`situation`).

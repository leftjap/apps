...
en 문장에 한국어 고유명사를 로마자로 쓰면(청기와 → `Cheonggiwa`) `src/services/koreanTerms.js` 에 `로마자: { ko, ipa }` 한 줄을 같이 추가한다. 없으면 TTS 가 영어 철자 그대로 읽는다 (2026-09-15). ipa 는 en-US 음소표 기호만 쓴다(테스트가 검사). 영어 철자 읽기가 이미 맞는 낱말(galbi 등)은 넣지 않는다. 문장은 한 voice 블록에서 한 사람이 읽고 이름만 IPA 로 바꾼다 — 한글 블록으로 나누는 방식은 억양이 끊겨 폐기했다 (2026-09-26). `validate-seed` 가 personal 트랙에서 사전에 없는 고유명사 후보를 FAIL 로 막고(문장 첫 낱말·붙임표 없는 소문자는 못 잡음), 발음은 `scripts/tts-probe-terms.mjs` 로 실측한다.

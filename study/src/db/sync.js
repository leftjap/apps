...
export async function pullTable(mapping, db, userId) {
  ...
  try {
    ...
    if (mapping.dexie === 'pronunciationLog' && typeof store.bulkGet === 'function') {
      ...
      // 2026-08-31 감점제 전환 표식 accuracyScore·scoreModel, 2026-09-01 축약 채택 표식 contractedRef 추가)
      const LOCAL_ONLY = ['prosodyIssues', 'wordScores', 'omissions', 'insertions', 'accuracyScore', 'scoreModel', 'contractedRef'];

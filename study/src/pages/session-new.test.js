 ...
 * 정본은 exLog[id].lastMain — 메인 녹음에서만 쌓인다 (sessionExprV2 applyScore). */
describe('session-new — restoreCardScore (카드 이동 시 점수링 복원)', () => {
  it('그 카드의 마지막 메인 점수를 돌려준다', () => {
    expect(restoreCardScore({ c1: { lastMain: 72 } }, 'c1')).toBe(72);
  });
  ...
  it('다른 카드 점수를 끌어오지 않는다', () => {
    expect(restoreCardScore({ c1: { lastMain: 72 } }, 'c2')).toBeNull();
  });

  it('숫자가 아닌 값은 null (구 스냅샷 방어)', () => {
    expect(restoreCardScore({ c1: { lastMain: null } }, 'c1')).toBeNull();
    expect(restoreCardScore({ c1: { lastMain: '72' } }, 'c1')).toBeNull();
  });
});

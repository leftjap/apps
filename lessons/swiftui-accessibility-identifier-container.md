...
**재발 2026-09-10 (gym)**: 홈 체중 카드 컨테이너에 `home-weight-card` 를 붙이자 안의
`home-weight-input`(기록하기 버튼) 식별자가 사라져 기존 UI 테스트가 깨졌다. 유산소 카드는
`cardio-card` 가 이미 자식 전부를 덮고 있어(히어로 `cardio-hero-*` 포함) 테스트가 "가장 큰 Text"
같은 우회로 요소를 찾고 있었다. 컨테이너 식별자는 처음부터 두지 않는 편이 낫다.

...
const BASE = new Date(2026, 5, 8); // 오늘 (목업)
...
function Header({ flags }){
  const [i,setI] = useState(sentenceOfDay());
  const s = SENTENCES[i];
    ...
    <header className="head">
      <div className="head__l">
        <div className="head__meta">
          ...
          <span>오늘</span>
          <span className="date mono">2026.{p2(t.m)}.{p2(t.d)} <b>{t.wd}</b></span>
        </div>
...
function Door({ habit, stateKey, demoMode, onDemo, isNext }){
  ...
  const launch = useCallback(()=>{
    if(demoMode){ onDemo(); return; }            // 데모: 상태 순환(앱 안 염)
    if(habit.url){
...
/* ---------- record (calendar) ---------- */
function Record({ stateKeys, period, order, onOpenStats }){
  const ticks = [period-1, Math.round((period-1)*0.74), Math.round((period-1)*0.5), Math.round((period-1)*0.26), 0].map(n=>dayMeta(n, BASE));
  // figures (수량 기반)
  const seqs = HABITS.map((h,i)=>fullSeq(h, h.states[stateKeys[i]]).slice(-period));
    ...
    <section className="rec">
      ...
      <div className="rec__rows">
        {(order || HABITS.map((_,i)=>i)).map(i=>{
          ...
          const seq=fullSeq(h,st).slice(-period);
          return (
...
function nextHabitId(stateKeys){
...
function StatsOverlay({ stateKeys, order, onClose }){
  useEffect(()=>{
      ...
      <div className="ov__panel" onClick={e=>e.stopPropagation()}>
        ...
        <div className="ov__rows">
          {(order||HABITS.map((_,i)=>i)).map(i=>{
            ...
            const longest=longestRun(fullSeq(h,st));
...
function App(){
  ...
  const ord = ORDER.map(id=>HABITS.findIndex(h=>h.id===id));
  const flags = ord.map(i=>HABITS[i].states[stateKeys[i]].kind==="done");
  const nextId = nextHabitId(stateKeys);
  ...
  return (
    <div className="page">
      ...
      <section className="doors">
        {ord.map(i=>(
          <Door key={HABITS[i].id} habit={HABITS[i]} stateKey={stateKeys[i]} demoMode={t.demoMode} onDemo={()=>demo(i)} isNext={HABITS[i].id===nextId} />
        ))}
      </section>
      {t.showRibbon && <DayRibbon stateKeys={stateKeys} />}
      {t.showRecord && <Record stateKeys={stateKeys} period={t.period} order={ord} onOpenStats={()=>setStatsOpen(true)} />}
      <p className="hint">
        {t.demoMode
      ...
      {statsOpen && <StatsOverlay stateKeys={stateKeys} order={ord} onClose={()=>setStatsOpen(false)} />}
    </div>
  );
}

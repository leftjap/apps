...
- **독서 데이터**: `book_reading_seconds`(일별 분) + `book_current_reading`(제목·저자·진도% — `~/.local/bin/millie-book-sync.sh` 가 맥 밀리앱 로컬 DB 에서 15분마다 적재, 2026-06-22~). hook=「제목」 N%까지, slot3=이번 달 시간. (구 서술 "제목·진도% 미연동"은 2026-07-04 폐기 — 연동됨.)
...
|---|---|---|
| 독서 read | `book_reading_seconds` + `book_current_reading` | `seconds`/60 (분) + 제목·진도%(millie-book-sync 적재). 직전=「제목」 N%까지, 이번 주=활동일/제안5일, 추세=이번 달 시간 |

...
**화면시간 데몬 (시스템, repo 밖 `~/.local/bin/`)**: `app-usage-sync.py`(앱=macOS knowledgeC `/app/usage`). **부풀림 2겹 제거**: (1) `∩ /display/isBacklit`(화면 켜짐) — 화면 OFF 중 기록 제거. (2) **`IDLE_CAP`(기본 300초) 캡** — knowledgeC record 는 '입력'이 아니라 '앱 전환'에서만 끊겨(같은앱 인접 record 0건 검증), 단일앱이 5분 넘게 무전환 지속 = 자리비움/영상(Chrome/Claude 가 wake-lock 으로 디스플레이를 못 자게 해 ∩isBacklit 로도 안 걸러짐). 시스템 기존 활성 임계(chrome-site-poll `IDLE_MAX=300`)에 맞춰 각 record 캡 → AFK 제거(스타크래프트 39h→6h·전체 156h→87h, 능동 Claude 는 거의 불변=올바른 차등제거). `APP_SYNC_DAYS`·`IDLE_CAP_SEC` env. + `chrome-site-poll.py`(사이트=30초 폴링, Chrome 최전면+`HIDIdleTime`<5분) + `.sh` 래퍼. launchd: `com.gio.app-usage-sync`(900초)·`com.gio.chrome-site-poll`(30초).
- **커버리지 정합**: 사이트는 chrome-site-poll 시작일(06-22)부터만 존재. 앱을 knowledgeC 로 그 이전까지 백필하면 "Chrome 한달 52h vs 사이트 4일 4h" 모순. 그래서 `TRACKING_START`(기본 2026-06-22) 이전 앱은 적재 안 함 — 앱·사이트 같은 기간만 표시(코히어런스 > 완전성). **`APP_SYNC_DAYS=35` 백필 재실행 금지**(TRACKING_START 이전은 어차피 스킵되나 의도 명확화). 추적이 06-22 시작이라 데이터<1주일이면 주==월(정상 — 누적되며 분기).

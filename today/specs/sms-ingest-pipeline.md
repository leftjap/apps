...
**⚠ 2026-06-02 화면 검증 — 배너 사각지대 확정:** 소연 계정 가계부를 실제로 띄워 검증한 결과 **신한 갭 배너가 안 뜬다**(`gaps=[]`, banner hidden, dismiss 아님). 신한 자동수집이 카드당 <10건(체크 3·Air 2)이라 "주력 카드" 임계 미달 → 배너 대상에서 제외. 즉 현 배너는 *"잘 되던 카드가 끊긴 경우"*만 잡고, **신한처럼 "처음부터 자동수집이 안 되던" 카드는 거래가 아무리 많아도(신한 1,425건) 못 잡는다.** 2026-05-26 배너 신설 의도(소연 신한 사고 재발 방지)가 정작 신한엔 무력. → **개선안**: 임계를 "자동수집 ≥10"이 아니라 "거래 ≥N건인데 자동수집 0/현저히 낮음" 기준을 추가하는 방향 검토 (코드 수정은 별도 승인 후).

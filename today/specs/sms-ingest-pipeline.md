...
**단축어 "Today 가계부 SMS"**:
1. `입력 받기` (Shortcut Input → 텍스트). `입력 없는 경우: 계속하기` (2026-06-11 지오 폰 변경 — 기존 "텍스트 요청"은 자동화 환경에서 침묵 정지. "계속하기"면 빈 입력도 POST돼 서버 400으로 관측 가능)
2. `URL 콘텐츠 가져오기`
...
- `ok` / `duplicate` / `unparsed` (amount_krw=NULL) / `rejected` (거절·취소·명세서·결제예정·광고·차단·비번·카드발급·0원외화·카드대금)
...
1. **입력 받기** (단축어 메타 설정 — 별도 액션 X, 단축어 자체 input)
   - 입력 유형: 텍스트 및 앱
   - 입력 없는 경우: **계속하기** (지오 2026-06-11 변경. 구 "텍스트 요청"은 백그라운드 자동화에서 프롬프트를 못 띄워 침묵 정지 — "계속하기"는 빈 입력이 서버 400 `Missing text`로 찍혀 silent failure 가 관측 가능해짐. 소연 폰은 미변경)
2. **URL의 콘텐츠 가져오기**
   - URL: `https://tcbooffrdacfatywdzcm.supabase.co/functions/v1/sms-card-ingest`
   - 메소드: POST
   - 헤더 2개:
...
|---|---|---|---|---|
| 신한8244 | 신한카드 Air | +8215447200 (1544-7200) | SMS | 키워드 "신한" (2026-06-11 전환) |
| 신한8579 | K-패스 신한카드 체크 | +8215447200 | SMS | 키워드 "신한" (2026-06-11 전환) |
| 신한8619 | K-패스 신한카드 체크 | +8215447200 (추정) | SMS | 키워드 "신한" (본문에 '신한' 포함 시) |
| 현대백화점카드 | (raw 그대로) | **+8215883650 (1588-3650)** | SMS | 키워드 "현대백화점카드" (2026-06-11 단일화) |
...
| Keep 식별자 (raw) | 친화 카드명 | 발신처 | 채널 | 잡는 자동화 |
|---|---|---|---|---|
| 삼성1337 | (raw 그대로) | +8215888900 (1588-8900) | SMS | 키워드 "1337" (2026-06-11 전환) |
| 삼성1337 해외승인 | (raw 그대로) | +82220008100 (02-2000-8100) | SMS | 키워드 "1337" |
| KB국민카드 7007 (후불하이패스 포함) | (raw 그대로) | +8215881688 (1588-1688) | SMS | 키워드 "KB국민카드" (2026-06-11 전환) |
| (외화·안내 fallback) | — | +821063491949 (지오 본인) | SMS | (자동화 제거 — 2026-06-11 키워드 전환으로 불필요) |
...
**지오 (leftjap@gmail.com)** — **키워드 기반 2종으로 전환 (2026-06-11, 발신번호 자동화 산발 드랍 10~13% + 6월 해외승인 2/2 누락 계기)**:
- 키워드 `1337` — 삼성 국내·해외·취소·후불교통(`[삼성카드]1337`) 전부 커버. 1년 실측(574건) 결제 재현율 100%
- 키워드 `KB국민카드` — KB 결제·해외·후불하이패스 커버. 1년 실측(42건) 결제 재현율 100% (미포함 12건 전부 명세서 링크)
- 구 발신번호 자동화(1588-8900 / 02-2000-8100 / 1588-1688 / 본인번호)는 삭제 — 신·구 동시 가동 시 received_at 차이로 unique 제약(owner_id, sms_raw, spent_at) 미차단 → 중복 행 가능. 단 소연 #2+#4 중복 캐치가 1건만 저장된 기록(상단 마스터 표)도 있어 메커니즘 미확정 — 안전측으로 삭제
- 부작용 차단: 키워드 전환으로 KB 카드대금 청구 알림(1899-0800, 월 1회꼴)이 유입되기 시작 → `isRejectPattern`에 `/카드대금/` 추가 (2026-06-11 배포, 이중계상 방지)
- **전환 검증 (2026-06-11 17:48)**: 키워드 자동화 + "1337" 포함 셀프 iMessage → 발화 + 본문 전달 + gateway 200 + reject 처리(행 0) — 전 구간 1회 완주 확인. ⚠ 직전 시도 3회(17:33·17:38·소연 발송)는 침묵 — 원인 미확정(신규 자동화 활성화 지연 vs 단축어 `입력 없는 경우: 계속하기` 변경 효과, 두 변수 동시 변경으로 구분 불가). **실 카드 SMS 첫 결제가 최종 확증** — 도달 실패 시 빈 입력이면 이제 400으로 관측됨

**소연 (soyoun312@gmail.com)** — **키워드 기반 2종으로 재구성 (2026-06-11)**:
- 키워드 `신한` — e2e 검증 완료 (21:08 테스트: 발화 + 본문 전달 + gateway 200 + reject 행0, 첫 시도 성공). 실결제 최종 확증 대기
- 키워드 `현대백화점카드` — 기존 작동 자동화 유지 (키워드 '현대백화점'으로 잠시 수정했다가 원복)
- 구 자동화(#1 발신번호 삼성+본인 / #3 키워드 "승인" / #4 발신번호 신한+현대) 전부 삭제 — 본인번호 잡문자 사고·"승인" 0건·신한 발신번호 미작동 이력 (아래 구 구성 분석 참조)
- ⚠ 소연 단축어 "입력 없는 경우: 계속하기" 변경 여부 미확인 (지오 폰만 적용 확정)

(이하 구 구성 분석 기록 — 2026-05-26 시점 4개 자동화:)
...
4. **발신번호**: `+8215447200` (신한카드, 2026-05-26 추가) + `+8215883650` (현대백화점카드, 2026-05-26 확정). 셋업 안내: [`specs/soyoun-shinhan-automation-setup.md`](soyoun-shinhan-automation-setup.md).
...
| #4 신한 발신번호 | 셋업 직후 1건 (test, 2026-05-26) — POST→INSERT→PWA 표시 e2e 통과 (스샷: 로컬 `handoff/verify-shinhan-edge-e2e.png` — 실거래 내역 포함이라 repo 제외) | **edge function·파서·DB·PWA 흐름은 통과**. 자동 트리거는 소연 폰 셋업 후 운영 관찰 필요. **2026-06-02 실측: 여전히 실시간 0건** (신한 1,250건 중 실시간 자동 ingest 0, count=exact) — 발신번호 1544-7200 미작동. 키워드 "신한" 전환 권장 |
...
### 빠른 진단 플로우 (2026-06-11 지오 사고로 표준화 — 증상: "결제했는데 앱에 없음")

```
0. 토큰 last_used_at (아래 절차 1)        → 마지막으로 서버에 닿은 시각
1. gateway 로그 (아래 절차 3')             → 결제 시각에 도달했나?
   ├ 0건  → 폰사이드 (자동화 미발화 / 단축어 정지 / 네트워크)
   ├ 400  → 자동화는 발화, 본문이 빈 채 도착 (입력 전달 문제)
   ├ 401  → 토큰/헤더 문제
   └ 200인데 앱에 없음 → status 확인 (rejected/unparsed?) + PWA 새로고침 (절차 4)
2. chat.db 대조 (지오 한정, 절차 아래 backfill 스크립트와 동일 추출기) → 누락 범위 확정 + 복구
3. 폰 테스트 계단 (싼 것부터):
   ⓐ 단축어 ▶ 수동 실행 + 텍스트 입력   → 단축어 본체·토큰·네트워크 권한
   ⓑ 키워드 포함 메시지 셀프 발송        → 자동화 발화 + 본문 전달
   ⓒ 실 카드 SMS (실결제)               → 최종 판정 (ⓑ와 거동 다를 수 있음)
```

⚠ 함정 (2026-06-11 실측): **신규 자동화는 생성 직후 침묵할 수 있음** (지오 폰 3회 침묵 → ~20분 후/단축어 폴백 변경 후 정상화, 원인 미확정) — 만들면 반드시 ⓑ로 발화 확인. **셀프 iMessage는 불완전한 대용물** — 침묵해도 실 SMS에선 될 수 있음. **단축어 "입력 없는 경우"는 "계속하기"여야** 침묵 정지 대신 400 신호가 남음.
...
- "주력 카드" 판정: 최근 90일 **자동수집(SMS-origin: `source==='sms' && card && sms_raw`) ≥10건** (구현: `src/features/expenses.js` `detectIngestGapCards`, `DOMINANT_COUNT=10`). ※ "거래 ≥10건"이 아니라 **자동수집 행 ≥10건** 기준임에 주의
- 노출: PWA 가계부 화면 상단 배너 (외부 인프라 무, 사용자 동선에 직접 노출)

**⚠ 2026-06-02 화면 검증 — 배너 사각지대 확정:** 소연 계정 가계부를 실제로 띄워 검증한 결과 **신한 갭 배너가 안 뜬다**(`gaps=[]`, banner hidden, dismiss 아님). 신한 자동수집이 카드당 <10건(체크 3·Air 2)이라 "주력 카드" 임계 미달 → 배너 대상에서 제외. 즉 현 배너는 *"잘 되던 카드가 끊긴 경우"*만 잡고, **신한처럼 "처음부터 자동수집이 안 되던" 카드는 거래가 아무리 많아도(신한 1,425건) 못 잡는다.** 2026-05-26 배너 신설 의도(소연 신한 사고 재발 방지)가 정작 신한엔 무력. → **개선 완료 (2026-06-02)**: `detectIngestGapCards`를 "자동수집 ≥10"에서 **"전체 거래 ≥10 + 자동수집 이력 ≥1 + 자동수집 7일+ 끊김"**으로 변경. 신한처럼 "거래는 많지만 자동수집이 적게만 되다 끊긴" 카드 감지. 삼성(자동수집 0건)·잡문자(card null)는 오탐 방지로 제외. 소연 실데이터 화면검증 — 수정 전 `gaps=[]` → 수정 후 신한 2종 배너 표시 확인("⚠ K-패스 신한카드 체크, 신한카드 Air 자동 수집 8일 무이력"). 단위테스트 6건 추가.
  - **남은 한계**: 자동수집 이력이 0건인 카드(한 번도 자동 안 됨)는 여전히 미감지 — "의도적 비자동(삼성 카톡)"과 구분 불가하기 때문. 신한은 backfill 5건 이력이 있어 감지됨.
...
5. **소연 신한 — 2026-06-02 실측 여전히 실시간 0건** (Air 446 + 체크 804 = 1,250건 중 실시간 자동 ingest 0건, count=exact). edge function·파서·두 파일 동일·토큰은 정상이나 소연 폰 신한 자동화가 미작동(발신번호 1544-7200 매칭 0 — 미설정/번호 불일치/비활성). 발신번호 우월 권장을 신한에 한해 정정 → **키워드 "신한" 자동화 권장** ([`specs/soyoun-shinhan-automation-setup.md`](soyoun-shinhan-automation-setup.md) v3). 8619는 today 실 SMS 미수집(DB 0, keep/spec 기록만 — 실 본문은 8244·8579 2종만 확인). **2026-06-03 정정**: keep(이전 앱)에선 신한 자동수집이 3/16~5/2 정상(34건)이었다 — "처음부터 불가"는 today-native 한정. keep은 발신번호 없이 본문만으로 처리했으므로 **발신번호는 불필요**, today 전환 때 발신번호 자동화로 잘못 재구성된 게 원인 (변경이력 2026-06-03)
...
| 2026-05-26 | 소연 신한 미수집 진단. 근본 원인: today-native 신한 수집은 처음부터 작동한 적 없음(0건), keep 수동 import이 stopgap 갭 가려주다 05-14 종료로 노출. fix: edge function 재배포(`50a80d7` 신한 파서 fix 반영), `_shared` deps git 추적, 소연 폰 자동화 #4 (신한 발신번호) 추가 안내. e2e POST→INSERT→PWA 검증 통과 (스샷: 로컬 `handoff/verify-shinhan-edge-e2e.png` — 실거래 내역 포함이라 repo 제외). spec 검증 기록 정정 + ingest health 임계치 신설 |
...
| 2026-06-11 | **지오 자동화 키워드 전환 + 카드대금 reject**. 폰 검증: 단축어 수동 실행(16:27·16:28 gateway 200) + 자동화 본인번호 셀프발송(16:36, 200 + 거절처리 행0 = 본문 전달 확인) — 둘 다 정상이나 실 카드 SMS(15:56)는 도달 0 → 조건부 실패(잠금/채널/사전 앱 실행 변수 미분리). 1년 chat.db 실측으로 키워드 재현율 검증(삼성 574건·KB 42건 — 결제 미포함 0, §자동화 트리거 참조) 후 발신번호→키워드(`1337`/`KB국민카드`) 전환 안내. 키워드 부작용(KB 카드대금 청구 알림 1899-0800 → unparsed 0원 행) 실측 → `isRejectPattern` `/카드대금/` 추가, `--no-verify-jwt` 재배포(v22). 배포 후 검증: 카드대금 본문 rejected·행0 + Authorization 없는 POST 200(verify_jwt 회귀 무) + 기존 결제 본문 duplicate 정상 파싱 |

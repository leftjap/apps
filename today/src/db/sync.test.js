...
describe('TABLE_MAP', () => {
  it('entries + expenses + merchant_rules + comments + notifications + reactions + user 매핑 3종', () => {
    expect(TABLE_MAP).toHaveLength(9);
    ...
    // 이모지 리액션 — RLS 가 본인+파트너 공유글 가시성, replace pull(하드삭제 반영)
    expect(TABLE_MAP[5].dexie).toBe('reactions');
    expect(TABLE_MAP[5].filterColumn).toBeNull();
    expect(TABLE_MAP[5].replace).toBe(true);
    // Wave 11.8 — 사용자별 매핑 3종 (admin UI 편집 대상), user_id 필터
    expect(TABLE_MAP[6].dexie).toBe('user_categories');
    expect(TABLE_MAP[6].filterColumn).toBe('user_id');
    expect(TABLE_MAP[7].dexie).toBe('user_brand_categories');
    expect(TABLE_MAP[7].filterColumn).toBe('user_id');
    expect(TABLE_MAP[8].dexie).toBe('user_merchant_aliases');
    expect(TABLE_MAP[8].filterColumn).toBe('user_id');
  });
